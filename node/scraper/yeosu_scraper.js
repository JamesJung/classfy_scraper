        ...
        while (retries < maxRetries) {
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const fileLinks = document.querySelectorAll('.guard ul li a');

                    fileLinks.forEach(link => {
                        // Exclude preview buttons
                        if (!link.classList.contains('preview_btn')) {
                            ...
                            if (matches && matches.length === 4) {
                                ...
                                const fullUrl = `https://eminwon.yeosu.go.kr/emwp/jsp/ofr/FileDownNew.jsp?user_file_nm=${encodeURIComponent(fileName)}&sys_file_nm=${encodeURIComponent(systemFileName)}&file_path=${encodeURIComponent(filePath)}`;
        ...
        try {
            ...
            if (attachment.onclick) {
                ...
                if (goDownloadMatch) {
                    ...
                    downloadUrl = `https://eminwon.yeosu.go.kr/emwp/jsp/ofr/FileDownNew.jsp?user_file_nm=${enc_user_file_nm}&sys_file_nm=${enc_sys_file_nm}&file_path=${enc_file_path}`;
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.yeosu.go.kr/www/govt/news/notify/new_notifys',
            required: true
        })
