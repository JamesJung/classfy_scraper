            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '#contents', '#board', '.boardViewWrap', '.contents_wrap', '.program--contents',
                    ...
                    const items = document.querySelectorAll('li.gosiWrap');

                    items.forEach(item => {
                        const fileNameAnchor = item.querySelector('a[title*="다운받기"]');

                        if (fileNameAnchor) {
                            const fileName = fileNameAnchor.textContent.trim();
                            const downloadLink = fileNameAnchor.href;
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.uiryeong.go.kr/board/list.uiryeong?boardId=BBS_0000070&menuCd=DOM_000000203003001001&contentsSid=201',
            required: true
        })
