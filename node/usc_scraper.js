            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    if (!dateText) {
                        ...
                        const details = document.querySelectorAll('.boardView .data dt');
                        let registrationDate = null;

                        details.forEach(dt => {
                            // '등록일' 텍스트를 가진 <dt> 요소를 찾습니다.
                            if (dt.textContent.trim() === '등록일') {
                                // 해당 <dt>의 바로 다음 형제 요소인 <dd>의 텍스트를 가져옵니다.
...
function setupCLI() {
    ...
    return yargs
        ...
        .option('url', {
            ...
            default: 'https://www.usc.go.kr/ko/page.do?pageNo=1&board_code=&srchSDate=&srchKwd=&srchColumn=&srchDept=&srchEDate=&mnu_uid=157&',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: 'table.tbl_board tbody tr'
        })
