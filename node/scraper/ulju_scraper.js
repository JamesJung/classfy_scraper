            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '#content', '#contents', '.bod_wrap', '.view', '.contents_wrap', '.program--contents',
                    ...
                    const goDownloadLinks = document.querySelectorAll('a[onclick*="goDownload_location"]');
    ...
    async downloadSingleAttachment(attachment, attachDir, index) {
        try {
            ...
            if (attachment.onclick) {
                // goDownload 패턴 처리
                const goDownloadMatch = attachment.onclick.match(/goDownload_location\(\s*['"]([^'"]*)['"]\s*,\s*['"]([^'"]*)['"]\s*,\s*['"]([^'"]*)['"]\s*\)/);
                ...
                if (goDownloadMatch) {
                    ...
                    downloadUrl = `https://eminwon.ulju.ulsan.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=${enc_user_file_nm}&sys_file_nm=${enc_sys_file_nm}&file_path=${enc_file_path}`;
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.ulju.ulsan.kr/ulju/saeol/gosi/list.do?seCode=01&mId=0403010000&token=1757575484448',
            required: true
        })
