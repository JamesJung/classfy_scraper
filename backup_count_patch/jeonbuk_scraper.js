...
class AnnouncementScraper {
            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '.bbs_view', 'table.skinTb-data-resList', '.program--contents',
                    ...
                    const fileContainer = document.querySelector('p.bbs_filedown');
                    const downloadLinks = fileContainer.querySelectorAll('a.ico_file');

                    downloadLinks.forEach(link => {
                        // A regex to match the file name and ignore the size in brackets.
                        // This looks for the text content of the node before the download link.
                        const fullTextNode = link.previousSibling;

                        // Use a regex to capture the file name, ignoring the size in brackets
                        const match = fullTextNode.textContent.match(/^\s*(.*?)\s*\[\d+\s*kb\]/i);

                        let fileName = '파일명 정보 없음';
                        if (match && match.length > 1) {
                            fileName = match[1].trim();
                        } else {
                            // Fallback in case the format changes
                            fileName = fullTextNode.textContent.trim();
                        }
                    ...
                    const fileContainers = document.querySelectorAll('.download li');

                    fileContainers.forEach(container => {


                        const fileNameElement = container.querySelector('span.fileNM');
                        const fileLinkElement = container.querySelector('a.mobaDown');

                        const fileName = fileNameElement.textContent.trim()
                        const downloadLink = fileLinkElement.href
                        const onclick = fileLinkElement.onclick
    ...
    async downloadSingleAttachment(attachment, attachDir, index) {
        try {
            ...
            if (attachment.onclick) {
                ...
                // goDownload 패턴 처리
                const goDownloadMatch = attachment.onclick.match(/pf_download\(\s*['"]([^'"]*)['"]\s*,\s*['"]([^'"]*)['"]\s*,\s*['"]([^'"]*)['"]\s*\)/);
    ...
    async buildDetailUrl(announcement) {
        ...
        if (announcement.onclick) {
            ...
            const match = announcement.onclick.match(/goPage\((\d+)\)/);

            if (match) {
                const detailKey = match[1]

                return `https://state.gwd.go.kr/portal/bulletin/notification?pageIndex=1&recordCountPerPage=15&mode=&firstYN=N&articleSeq=${detailKey}&searchCondition=TITLE&searchKeyword=`
            }
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.jeonbuk.go.kr/board/list.jeonbuk?boardId=BBS_0000129&menuCd=DOM_000000102002005000&contentsSid=1379&cpath=',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: 'table.bbs_table > tbody > tr'
        })
        .option('title-selector', {
            type: 'string',
            description: '제목 선택자',
            default: 'td:nth-child(4) a'
        })
