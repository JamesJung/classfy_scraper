...
class AnnouncementScraper {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    if (!dateText) {
                        ...
                        if (announcement && announcement.listDate) {
                            ...
                            const dateElement = document.querySelector('.bbs--view--opt span:nth-child(4)');
                    ...
                    const linkElement = document.querySelector('div.ui.bbs--view--file a');
                    const fileLinks = document.querySelectorAll('div.ui.bbs--view--file a');
                    console.log("linkElement", linkElement)
                    if (fileLinks) {
                        fileLinks.forEach(link => {
                            // The file name is the text content of the link, excluding the image alt text.
                            // A simple textContent trim should work.
                            const url = link.href;
                            // Get the file name from the title attribute and remove " 다운로드".
                            const fileName = link.getAttribute('title').replace(' 다운로드', '').trim();
    ...
    extractDate(dateText) {
        ...
        // "계속"이라는 텍스트가 있으면 오늘 날짜로 처리
        if (cleanText.includes('계속') || cleanText === '계속') {
    ...
    buildListUrl(pageNum) {
        ...
        url.searchParams.set('GotoPage', pageNum);
        return url.toString();
    }
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.geumsan.go.kr/kr/html/sub03/030302.html',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: 'table.table-bordered tbody tr'
        })
