    ...
    async downloadViaEgovPost(fileNm, sysFileNm, filePath, attachDir, fileName) {
        try {
            ...
            const execResult = await this.page.evaluate((params) => {
                ...
                try {
                    ...
                    if (typeof fn_egov_downFile === 'function') {
                        ...
                        form.id = 'fileForm';
                        form.method = 'post';
                        form.action = 'https://eminwon.gokseong.go.kr/emwp/jsp/ofr/FileDownNew.jsp';
                        form.target = '_self';
            ...
            try {
                ...
                // 다운로드 실패시 URL 정보는 반환
                const downloadUrl = `https://eminwon.gokseong.go.kr/emwp/jsp/ofr/FileDownNew.jsp?user_file_nm=${encodeURIComponent(fileNm)}&sys_file_nm=${encodeURIComponent(sysFileNm)}&file_path=${encodeURIComponent(filePath)}`;
                return {
                    success: false,
                ...
                for (const { name, method } of downloadMethods) {
                    try {
                        ...
                        if (downloadResult && downloadResult.success) {
                            ...
                            return {
                                ...downloadResult,
                                actualDownloadUrl: downloadResult.url || 'https://eminwon.gokseong.go.kr/emwp/jsp/ofr/FileDownNew.jsp',
                                fileName: fileName
                            };
                        }
                    } catch (error) {
    ...
    async buildDetailUrl(announcement) {
        ...
        if (announcement.onclick) {
            ...
            if (viewMatch) {
                ...
                const not_ancmt_mgt_no = viewMatch[1];
                let url = `https://www.gokseong.go.kr/board/GosiView.do?menuNo=102001003000&pageIndex=1&searchCnd=0&not_ancmt_mgt_no=${not_ancmt_mgt_no}&not_ancmt_se_code=&list_gubun=&srhCate=title&searchWrd=`
                return url;
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.gokseong.go.kr/board/GosiList.do?menuNo=102001003000',
            required: true
        })
