...
class AnnouncementScraper {
        ...
        while (retries < maxRetries) {
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const goDownloadLinks = document.querySelectorAll('dl.file a');
                    ...
                    goDownloadLinks.forEach(link => {
                        ...
                        console.log("fileName", fileName, url, "target:", target, "onclick:", onclick)
                        if (fileName && url) {
                            attachments.push({
                                name: fileName,
                                url: url,
    ...
    async downloadSingleAttachment(attachment, attachDir, index) {
        try {
            ...
            console.log(`onclick: ${attachment.onclick || 'none'}`);
            console.log(`target: ${attachment.target || 'none'}`);

            let downloadUrl = attachment.url;
            let fileName = attachment.name || `attachment_${index}`;

            // target="_blank" 처리를 위한 브라우저 방식 다운로드
            if (attachment.target === '_blank' || attachment.onclick) {
                console.log('🔄 브라우저 방식 다운로드 시작...');
                await this.downloadViaBrowser(attachment, attachDir, fileName);
                return;
            }
            ...
            if (attachment.onclick) {
                ...
                if (goDownloadMatch) {
                    ...
                    downloadUrl = `https://eminwon.gurye.go.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=${enc_user_file_nm}&sys_file_nm=${enc_sys_file_nm}&file_path=${enc_file_path}`;
            ...
            return {
                [fileName]: {
                    ...
                    downloadType: attachment.target === '_blank' || attachment.onclick ? 'browser' : 'direct',
                    fileName: fileName
                }
            };
        } catch (error) {
    ...
    /**
     * 브라우저를 통한 다운로드 (target="_blank" 링크 처리)
     */
    async downloadViaBrowser(attachment, attachDir, fileName) {
        try {
            ...
            const downloadPromise = new Promise((resolve, reject) => {
                ...
                const downloadHandler = async (download) => {
                    try {
                        ...
                        const safeFinalFileName = sanitize(finalFileName, { replacement: '_' });
                        const savePath = path.join(attachDir, safeFinalFileName);

                        console.log('🔍 다운로드 이벤트 감지:', {
                            originalUrl: attachment.url,
                            actualDownloadUrl: actualDownloadUrl,
                            suggestedFileName,
                            finalFileName: safeFinalFileName,
                            savePath
                        });
                        ...
                        while (attempts < maxAttempts) {
                            try {
                                ...
                                if (tempPath && (await fs.pathExists(tempPath))) {
                                    console.log(`✅ 임시 파일 확인됨: ${tempPath}`);
                                    break;
                                }
                            } catch (pathError) {
                                // 실패 공고 DB 기록
                                await FailureLogger.logFailedAnnouncement({
                                    ...
                                    error_message: pathError.message
                                }).catch(logErr => {});
                        ...
                        if (!tempPath || !(await fs.pathExists(tempPath))) {
                            throw new Error(`임시 파일 접근 실패: ${tempPath}`);
                        }
            ...
            // 링크 클릭 실행 - target="_blank" 링크의 경우 특별 처리
            if (attachment.onclick) {
                console.log('onclick 이벤트 실행...');
                await this.page.evaluate((onclick) => {
                    eval(onclick);
                }, attachment.onclick);
            } else if (attachment.url && attachment.target === '_blank') {
                console.log('target="_blank" 링크 처리 중...');
                // target="_blank" 링크를 현재 탭에서 열기 위해 JavaScript로 처리
                await this.page.evaluate((url) => {
                    // 새 창 대신 현재 창에서 다운로드 실행
                    window.location.href = url;
                }, attachment.url);
            } else if (attachment.url) {
                console.log('URL로 직접 이동...');
                await this.page.goto(attachment.url, {
                    waitUntil: 'networkidle',
                    timeout: 30000
                });
            } else {
                throw new Error('다운로드할 수 있는 URL이나 onclick이 없습니다');
            }

            // 다운로드 완료 대기
            const result = await downloadPromise;
            console.log(`✅ 브라우저 다운로드 성공: ${result.savedPath}`);
            ...
            return {
                [fileName]: {
                    ...
                    originalOnclick: attachment.onclick,
                    actualDownloadUrl: result.actualDownloadUrl,
    ...
    buildListUrl(pageNum) {
        ...
        url.searchParams.set('pgno', pageNum);
        return url.toString();
    }
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.ganghwa.go.kr/open_content/main/ganghwa/news/announce.jsp',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: 'table.general_board tbody tr'
        })
