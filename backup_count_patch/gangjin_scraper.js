...
class AnnouncementScraper {
    ...
    async scrape() {
        try {
            ...
            // 기존 폴더의 제목들을 processedTitles에 추가 (force 옵션이 없는 경우에만)
            if (!this.force) {
                ...
                try {
                    ...
                    for (const announcement of announcements) {
                        try {
                            ...
                            // 브라우저 연결 문제인 경우 재초기화 시도
                            if (announcementError.message && 
                                (announcementError.message.includes('브라우저') || 
                                 announcementError.message.includes('Target closed') ||
                                 announcementError.message.includes('Protocol error'))) {
                                console.log('브라우저 연결 문제 감지, 재초기화 시도...');
                                try {
                                    await this.cleanup();
                                    await this.initBrowser();
                                } catch (reinitError) {
                                    ...
                                    await FailureLogger.logFailedAnnouncement({
                                        ...
                                        url: announcement?.link || announcement?.url,
                                        detail_url: announcement?.detailUrl,
                                        error_type: 'reinitError',
                                        error_message: reinitError.message
                                    }).catch(logErr => {});

                                    console.error('브라우저 재초기화 실패:', reinitError.message);
                                    shouldContinue = false;
                                    break;
                                }
                            }
    ...
    async getAnnouncementList(pageNum) {
        ...
        while (retries < maxRetries) {
            try {
                ...
                // 동적 컨텐츠 로딩 대기 및 선택자 대기
                try {
                    await this.page.waitForSelector(this.listSelector, { timeout: 10000 });
                } catch (e) {
                    console.log('리스트 선택자 대기 실패, 계속 진행...');
                }
        ...
        while (retries < maxRetries) {
            try {
                ...
                try {
                    ...
                    console.log('상세 페이지 이동 중 오류, 재시도...');
                    await this.delay(2000);
                    await this.page.goto(detailUrl, {
                        waitUntil: 'load',
                        timeout: 60000
    ...
    async downloadViaLink(url, attachDir, fileName) {
        try {
            console.log("downloadViaLink !!다운로드 시작:", url);
    ...
    async cleanup() {
        if (this.browser) {
            try {
                // 컨텍스트 및 브라우저 닫기
                if (this.context) {
                    await this.context.close().catch(e => console.warn('컨텍스트 닫기 오류:', e.message));
                }

                await this.browser.close().catch(e => console.warn('브라우저 닫기 오류:', e.message));
                console.log('\\n브라우저 정리 완료');
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.gangjin.go.kr/www/government/notice/gosi',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: '.board_list table.board_basic tbody tr'
        })
