...
class EminwonIndexer:
    ...
    def delete_existing_data(self, specific_regions=None):
        ...
        if specific_regions:
            ...
            placeholders = ', '.join(['%s'] * len(specific_regions))
            delete_sql = f"DELETE FROM eminwon_url_registry WHERE region IN ({placeholders})"
            cursor.execute(delete_sql, specific_regions)
            deleted = cursor.rowcount
            print(f"Deleted {deleted} records for regions: {', '.join(specific_regions)}")
        else:
            # Delete all data
    ...
    def run_indexing(self, specific_regions=None):
        ...
        if specific_regions:
            ...
            for region_name in specific_regions:
                ...
                if region_path.exists() and region_path.is_dir():
                    ...
                    print(f"⚠️  Warning: Region '{region_name}' not found in {self.data_dir}")
        ...
        for region_dir in tqdm(region_dirs, desc="Indexing regions"):
            ...
            if count > 0 or duplicates > 0 or errors > 0:
                tqdm.write(f"✓ {region_name:15} - New: {count:4}, Duplicates: {duplicates:4}, Errors: {errors:4}")
    ...
    def show_statistics(self, specific_regions=None):
        ...
        if specific_regions:
            ...
            cursor.execute(f"SELECT COUNT(*) FROM eminwon_url_registry WHERE region IN ({placeholders})", specific_regions)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Index existing eminwon data to database')
    parser.add_argument('-r', '--regions', nargs='+', help='Specific regions to index (e.g., -r 청주 수원)')
    parser.add_argument('--force', action='store_true', help='Force re-indexing by deleting existing data')
    ...
    if args.force:
        if args.regions:
            ...
            response = input("This will delete existing data for these regions. Continue? (y/n): ")
        else:
            print("Force re-indexing ALL regions")
            response = input("This will delete ALL existing indexed data. Continue? (y/n): ")
    ...
    print("Run 'python eminwon_incremental_crawler.py' to start collecting new announcements.")
