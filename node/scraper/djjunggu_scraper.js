...
class AnnouncementScraper {
            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '#conts', '#contBody', '.board-view-s1', '.bbs--view', '.program--contents',
                    ...
                    // fn_egov_downFile 패턴 찾기 - href 속성에서도 찾기
                    const fnFileDownLinks = document.querySelectorAll('a[href*="fn_egov_downFile"]');

                    console.log("fnFileDownLinks", fnFileDownLinks.length)
                    fnFileDownLinks.forEach(link => {
                        const href = link.getAttribute('href');
                        const fileName = link.textContent.trim();

                        // 미리보기 링크는 제외
                        if (fileName === '미리보기' || href.includes('preview')) {
                            return;
                        }

                        console.log("fileName", fileName, href)
                        if (fileName && href && href.includes('fn_egov_downFile')) {
                            ...
                            if (matches) {
                                ...
                                console.log("Added attachment:", fileName, downloadUrl);
                            }
                        }
                    });
    ...
    async downloadViaEgovPost(userFileNm, sysFileNm, filePath, attachDir, fileName, encodedOriginalFileName) {
        try {
            ...
            // URL과 원본 파일명 추가
            const downloadUrl = 'https://eminwon.djjunggu.go.kr/emwp/jsp/ofr/FileDownNew.jsp';
    ...
    async downloadViaAxiosWithSession(url, userFileNm, sysFileNm, filePath, attachDir, displayFileName, cookieString, referer) {
        try {
            ...
            const downloadUrl = url || 'https://eminwon.djjunggu.go.kr/emwp/jsp/ofr/FileDownNew.jsp';
     ...
     * 직접 URL 다운로드
     */
    async downloadViaDirectUrl(url, attachDir, fileName) {
        try {
            console.log("직접 URL 다운로드 시작:", url);
            ...
            const response = await axios({
                ...
                headers: {
                    ...
                    'Accept-Language': 'ko-KR,ko;q=0.9,en;q=0.8',
                    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
                    'Referer': 'https://www.djjunggu.go.kr/',
            ...
            // 파일명 정리
            const cleanFileName = sanitize(finalFileName, { replacement: '_' });
                ...
                writer.on('finish', async () => {
                    ...
                    if (stats.size < 200) {
                        try {
                            ...
                            if (content.includes('<!DOCTYPE') || content.includes('<html') || content.includes('오류')) {
                                await fs.unlink(filePath);
                                reject(new Error('에러 페이지가 다운로드됨'));
                                return;
                            }
                        } catch (e) {
                            // 바이너리 파일일 수 있으므로 무시
            ...
            console.error('직접 URL 다운로드 실패:', error.message);
            throw error;
        }
    }
    ...
    generateMarkdownContent(announcement, detailContent) {
        ...
        if (detailContent.attachments && detailContent.attachments.length > 0) {
            ...
            detailContent.attachments.forEach((att, i) => {
                ...
                // javascript:로 시작하는 경우는 제외
                if (att.url && !att.url.startsWith('javascript:')) {
                    lines.push(`${i + 1}. ${att.name}:${att.url}`);
                } else {
                    // javascript: URL인 경우 표시하지 않거나 원본 표시
                    lines.push(`${i + 1}. ${att.name}:(다운로드 불가)`);
                }
            });
        }
    ...
    async buildDetailUrl(announcement) {
        ...
        if (announcement.onclick) {
            ...
            const opDetailMatch = announcement.onclick.match(/fn_search_detail\s*\(\s*(\d+)\s*\)/);

            const regex = /fn_search_detail\('([^']+)'\)/;
            const matches = announcement.onclick.match(regex);

            if (matches) {
                console.log("!!!!!!!!!!!!!!opDetailMatch!!!!!!!")
                const mgtno = matches[1];
                const detailUrl = `https://www.djjunggu.go.kr/prog/saeolGosi/GOSI/sub03_06/view.do?notAncmtMgtNo=${mgtno}`;
                return detailUrl;
            }
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.djjunggu.go.kr/prog/saeolGosi/GOSI/sub03_06/list.do',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: '.no-more-tables table.table-default tbody tr'
        })
