...
class AnnouncementScraper {
    constructor(options = {}) {
        ...
        this.goPage = options.goPage || 1;  // 시작 페이지 추가
    ...
    async scrape() {
        try {
            ...
            let currentPage = this.goPage || 1;  // 시작 페이지 설정
        ...
        while (retries < maxRetries) {
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    // 첨부파일 링크 추출
                    const fileContainers = document.querySelectorAll('.file-item-container');
                    const attachments = [];

                    fileContainers.forEach(container => {
                        // Find the anchor tag for the file link within each container.
                        const fileLinkElement = container.querySelector('a[href*="FileDownNew.jsp"]');
                        // Find the span for the original file name.
                        const fileNameSpan = container.querySelector('.orignlFileNm');

                        if (fileLinkElement && fileNameSpan) {
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.gwangjin.go.kr/portal/bbs/B0000378/list.do?menuNo=200192',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: 'div.table table tbody tr'
        })
