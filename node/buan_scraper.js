            ...
            try {
                ...
                const announcements = await this.page.evaluate((selectors) => {
                    ...
                    rows.forEach((row, index) => {
                        ...
                        console.log("titleElement", titleElement, selectors.titleSelector)
                        console.log("dateElement", dateElement, selectors.dateSelector)
            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    if (!dateText) {
                        ...
                        const postInfo = document.querySelector('.bbs_vtop .col');
                        let postDate = null;

                        if (postInfo) {
                            const listItems = postInfo.querySelectorAll('li');
                            listItems.forEach(li => {
                                const strongTag = li.querySelector('strong');
                                if (strongTag && strongTag.textContent.trim() === '작성일') {
                                    // "작성일 : 2025.09.09" 에서 "2025.09.09"만 추출
                                    dateText = li.textContent.replace('작성일 :', '').trim();
                                }
                            });
                        }
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.buan.go.kr/board/list.buan?boardId=BBS_0000054&menuCd=DOM_000000103001003000&paging=ok&startPage=1',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: 'table.bbs_list_t tbody tr'
        })
