...
class AnnouncementScraper {
        ...
        while (retries < maxRetries) {
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const fileItems = document.querySelectorAll('.list_bu_sub li');

                    fileItems.forEach(item => {
                        const downloadAnchor = item.querySelector('a.ico_hwp, a.ico_pdf');
                        const fileNameSpan = item.querySelector('.fileName');

                        if (downloadAnchor && fileNameSpan) {
                            const fileName = fileNameSpan.textContent.trim();
                            // getAttribute를 사용하여 원본 href 값 가져오기
                            let downloadLink = downloadAnchor.getAttribute('href');

                            // 상대 경로인 경우 전체 URL로 변환
                            if (downloadLink && !downloadLink.startsWith('http')) {
                                downloadLink = new URL(downloadLink, window.location.href).toString();
                            }
    ...
    async downloadSingleAttachment(attachment, attachDir, index) {
        try {
            ...
            // URL에 공백이나 한글이 있으면 인코딩
            if (downloadUrl) {
                console.log('원본 URL:', downloadUrl);
                // 공백을 %20으로 치환
                downloadUrl = downloadUrl.replace(/ /g, '%20');

                // 한글이 있는 경우 추가 처리
                if (/[한글ㄱ-ㅣ가-힣]/g.test(downloadUrl)) {
                    try {
                        // URL 구성요소 분리 및 인코딩
                        ...
                        console.log('URL 인코딩 실패, 공백만 처리');
                        // 이미 공백은 %20으로 변환되었음
                    }
                }
                console.log('인코딩된 URL:', downloadUrl);
            }
    ...
    async downloadViaLink(url, attachDir, fileName) {
        try {
            ...
            console.log(`다운로드 URL: ${encodedUrl}`);

            // 파일명에 공백이나 특수문자가 있는 경우 Playwright로 다운로드
            if (fileName && (fileName.includes(' ') || fileName.includes('[') || fileName.includes(']'))) {
                console.log('특수 문자 포함 파일명 감지, Playwright로 다운로드:', fileName);

                // Playwright의 다운로드 이벤트 처리
                ...
                console.log(`파일 다운로드 완료 (Playwright): ${cleanFileName}`);

                return;
            }

            // 일반 파일은 기존 방식 사용
            // node-fetch 사용하여 다운로드
            ...
            console.log(`파일 다운로드 완료: ${cleanFileName}`);
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.jungnang.go.kr/portal/bbs/list/B0000117.do?menuNo=200475',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: 'table.inc_head tbody tr'
