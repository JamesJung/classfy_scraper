...
class AnnouncementScraper {
            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '#content', '#contBody', '.board_view', '.contents_wrap', '.program--contents',
                        '.bbs_skin',
                        '.main', '.article', '.post',
                        '#main', '#article', '#post',
                    ...
                    // 첨부파일 링크 추출
                    const listItems = document.querySelectorAll('.add_file ul li');
                    const attachments = [];

                    listItems.forEach(item => {
                        // 다운로드 링크를 가진 <a> 태그를 찾습니다.
                        const downloadLinkElement = item.querySelector('a[href*="FileDown"]');
                        // 파일 크기를 가진 <span> 태그를 찾습니다.
                        const fileSizeElement = item.querySelector('.byte');

                        if (downloadLinkElement) {
                            // <a> 태그의 전체 텍스트 콘텐츠를 가져옵니다.
                            let fileName = downloadLinkElement.textContent.trim();

                            // 파일 크기 <span>이 있다면 해당 텍스트를 파일 이름에서 제거합니다.
                            if (fileSizeElement) {
                                const fileSize = fileSizeElement.textContent.trim();
                                fileName = fileName.replace(fileSize, '').trim();
                            }

                            // 다운로드 링크를 href 속성에서 가져옵니다.
    ...
    extractDate(dateText) {
        ...
        // HH:MM:SS 형식인 경우 오늘 날짜로 설정
        const timeOnlyMatch = cleanText.match(/^(\d{1,2}):(\d{2}):(\d{2})$/);
        if (timeOnlyMatch) {
            console.log(`시간만 있음 (${cleanText}), 오늘 날짜로 설정`);
            return moment(); // 오늘 날짜 반환
        }
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.icdonggu.go.kr/main/information/news/announce.jsp',
            required: true
        })
