...
class AnnouncementScraper {
                ...
                const announcements = await this.page.evaluate((selectors) => {
                    ...
                    rows.forEach((row, index) => {
                        ...
                        if (titleElement && dateElement) {
                            ...
                            const dataSn = titleElement.getAttribute('data-sn');
            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '#contents', 'tbody.p-table--th-left', '.board-view', '.cont_inner', '.program--contents',
                        '#user_board_whole',
                    ];
                    ...
                    const fileLinks = document.querySelectorAll('a[onclick*="goDown"]');
    ...
    async downloadSingleAttachment(attachment, attachDir, index) {
        try {
            ...
            if (attachment.onclick) {
                // 관악구청 fileDown 패턴 처리
                const fileDownMatch = attachment.onclick.match(/goDown\(\s*['"]([^'"]*)['"]\s*,\s*['"]([^'"]*)['"]\s*,\s*['"]([^'"]*)['"]\s*\)/);
                if (fileDownMatch) {
                    // <input type="hidden" name="user_file_nm" id="user_file_nm" value="">
                    //     <input type="hidden" name="sys_file_nm" id="sys_file_nm" value="">
                    //         <input type="hidden" name="file_path" id="file_path" value="">
                    const [, originalName, serverName, serverPath] = fileDownMatch;
                    ...
                    downloadUrl = `https://eminwon.gwangsan.go.kr/emwp/jsp/ofr/FileDownNew.jsp?user_file_nm=${enc_user_file_nm}&sys_file_nm=${enc_sys_file_nm}&file_path=${enc_file_path}`;
                ...
                // goDownload 패턴 처리
                    ...
                    if (goDownloadMatch) {
                        ...
                        const enc_sys_file_nm = encodeURIComponent(serverName);
                        const enc_file_path = encodeURIComponent(serverPath);
                        downloadUrl = `https://eminwon.gwangsan.go.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=${enc_user_file_nm}&sys_file_nm=${enc_sys_file_nm}&file_path=${enc_file_path}`;
                        actualDownloadUrl = downloadUrl;
    ...
    async buildDetailUrl(announcement) {
        console.log('URL 구성 중:', {
            ...
            dataSn: announcement.dataSn
        });

        // e.preventDefault();
        // var $form = $('#searchForm');
        // $form.find("#notSn").val($(this).data('sn'));
        // $form.attr('action', '/notView.do?notSn=' + $(this).data('sn') + '&pageId=www12');
        // $form.submit();

        if (announcement.dataSn) {
            const sn = announcement.dataSn
            const detailUrl = `https://www.gwangsan.go.kr/notView.do?notSn=${sn}&pageId=www12`;
            return detailUrl;
    ...
    buildListUrl(pageNum) {
        ...
        url.searchParams.set('movePage', pageNum);
        return url.toString();
    }
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.gwangsan.go.kr/notList.do?pageId=www12&searchNotSe=01',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: 'table.type_noline tbody tr'
        })
        .option('title-selector', {
            type: 'string',
            description: '제목 선택자',
            default: 'td.subject a, td.tl a, td:nth-child(3) a'
        })
