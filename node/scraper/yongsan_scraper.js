        ...
        while (retries < maxRetries) {
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const goDownloadLinks = document.querySelectorAll('.file-list--set a.file');

                    console.log("goDownloadLinks", goDownloadLinks)
                    goDownloadLinks.forEach(link => {
                        const fileName = link.textContent.trim().split('[용량:')[0].trim();
                        const url = link.href;
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.yongsan.go.kr/portal/bbs/B0000095/list.do?optn1=01&pageUnit=&menuNo=200233&sdate=&edate=&searchWrd=&searchCnd=',
            required: true
        })
