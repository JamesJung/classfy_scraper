...
class AnnouncementScraper {
            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        'div.article-view', '.content_box', '.program--contents',
                    ...
                    console.log(document.querySelectorAll('table-article'))

                    console.log(document.querySelectorAll('ul.file-list'))
                    console.log(document.querySelectorAll('ul.file-list li'))

                    const fileList = document.querySelectorAll('ul.file-list li');

                    console.log("listItems", fileList)
                    fileList.forEach(item => {
                        // Find the anchor tag for the download link.
                        const downloadLinkElement = item.querySelector('a');
                        // Find the span tag for the file name.
                        const fileNameElement = item.querySelector('a span');

                        console.log(downloadLinkElement, fileNameElement)

                        if (downloadLinkElement && fileNameElement) {
    ...
    async buildDetailUrl(announcement) {
        ...
        if (announcement.link) {
            ...
            const viewMatch = announcement.link.match(/opView\(\s*['"]([^'"]*)['"]\s*\)/);
            ...
            if (viewMatch) {
                ...
                let viewUrl = `https://www.nowon.kr/www/user/bbs/BD_selectBbs.do?q_bbsCode=1003&q_bbscttSn=${keyValue}&q_estnColumn7=&q_estnColumn1=11&q_ntceSiteCode=11&q_clCode=0&q_rowPerPage=10&q_currPage=1&q_sortName=&q_sortOrder=&q_searchKeyTy=sj___1002&q_searchVal=&`
                return viewUrl
            }
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.nowon.kr/www/user/bbs/BD_selectBbsList.do?q_bbsCode=1003&q_clCode=0&q_estnColumn1=11&q_ntceSiteCode=11',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: 'table.table-list tbody tr'
