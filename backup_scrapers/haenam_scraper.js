...
class AnnouncementScraper {
    ...
    async getDetailContent(announcement) {
        ...
        while (retries < maxRetries) {
            try {
                ...
                // haenam 사이트는 onclick 함수 실행이 필요함
                if (announcement.onclick && announcement.onclick.includes('goDetialView')) {
                    console.log('Haenam onclick 방식으로 상세 페이지 접근');
                    
                    // 리스트 페이지에서 직접 클릭
                    const listPageNum = announcement.onclick.match(/goDetialView\('[^']+',\s*'([^']+)'/)?.[1] || '1';
                    const listUrl = this.buildListUrl(listPageNum);
                    ...
                    const clicked = await this.page.evaluate((params) => {
                        const { onclick, title } = params;
                        // 해당 제목의 링크 찾기
                        const links = document.querySelectorAll('a');
                        for (const link of links) {
                            if (link.textContent && link.textContent.includes(title.substring(0, 20))) {
                                // onclick 함수 실행
                                if (link.onclick) {
                                    link.click();
                                    return true;
                                ...
                                if (onclickAttr && onclickAttr.includes('goDetialView')) {
                                    eval(onclickAttr);
                                    return true;
                                }
                            }
                        ...
                        // 직접 onclick 실행
                        if (typeof goDetialView === 'function') {
                            eval(onclick);
                            return true;
                        }
                        return false;
                    }, { onclick: announcement.onclick, title: announcement.title });
                    
                    if (clicked) {
                        console.log('상세 페이지 클릭 성공');
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '#content', '#contens', '.view_box', '.contents_wrap', '.program--contents',
                        '.bbs_skin', '.view_box',
                    ...
                    const fileSelectors = [
                        '.filelist dd a',
                        'td a[href*="FileDown.jsp"]',  // 직접 다운로드 링크
                        'a[href*=".hwp"], a[href*=".pdf"], a[href*=".xlsx"], a[href*=".docx"]'  // 파일 확장자로 찾기
                    ];
                    ...
                    fileSelectors.forEach(selector => {
                        ...
                        items.forEach(item => {
                            const fileName = item.textContent.trim().replace(/\s+\(.*\)/, '');
                            const url = item.href;
                            const onclick = item.getAttribute('onclick');

                            // 중복 체크
                            const fileKey = fileName + (url || onclick || '');
                            if (fileName && !foundFiles.has(fileKey)) {
                                foundFiles.add(fileKey);
                                ...
                                // URL이 javascript:void(0) 같은 경우 null로 처리
                                if (attachmentInfo.url && attachmentInfo.url.includes('javascript:')) {
                                    attachmentInfo.url = null;
                                }
                ...
                // 현재 페이지 URL 가져오기 (onclick으로 이동한 경우)
        ...
        try {
            ...
            if (attachment.onclick) {
                ...
                if (goDownloadMatch) {
                    ...
                    downloadUrl = `https://eminwon.haenam.go.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=${enc_user_file_nm}&sys_file_nm=${enc_sys_file_nm}&file_path=${enc_file_path}`;
    ...
    async buildDetailUrl(announcement) {
        ...
        if (announcement.onclick) {
            ...
            if (!matches) {
                console.log('goDetialView 패턴 매칭 실패');
                return null;
            }
            ...
            // haenam 사이트의 상세 페이지는 POST 방식으로 접근해야 함
            // 직접 상세 페이지 URL 구성 (GET 방식 시도)
            const detailUrl = `https://www.haenam.go.kr/index.9is?contentUid=18e3368f7913117f01791bdc63505ada&notmgtNoView=${id}&nowPageNum=${num}&searchType=${type}&keyword=${key}&recordCountPerPage=10`;
    ...
    buildListUrl(pageNum) {
        ...
        url.searchParams.set('nowPageNum', pageNum);
        return url.toString();
    }
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.haenam.go.kr/index.9is?contentUid=18e3368f7913117f017915ea3b971122&nowPageNum=1&recordCountPerPage=10&searchType=&keyword=',
            required: true
        })
