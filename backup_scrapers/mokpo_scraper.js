                ...
                const announcements = await this.page.evaluate((selectors) => {
                    ...
                    rows.forEach((row, index) => {
                        ...
                        if (titleElement && dateElement) {
                            const title = titleElement.textContent.trim().replace("새로운글", "");
            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        ...
                        '.bbs_skin', '#board_basic_view', '.module_view_box', '.bbs_view',
                        '.main', '.article', '.post',
                        '#main', '#article', '#post',
                    ...
                    fileItems.forEach(item => {
                        ...
                        if (downloadLink) {
                            ...
                            if (matches && matches.length === 4) {
                                ...
                                const fullUrl = `https://eminwon.mokpo.go.kr/emwp/jsp/ofr/FileDownNew.jsp?user_file_nm=${enc_user_file_nm}&sys_file_nm=${enc_sys_file_nm}&file_path=${enc_file_path}`;
                    ...
                    //////
                    // const fileItems = document.querySelectorAll('.file_viewbox ul li');

                    // fileItems.forEach(item => {
                    //     // 각 항목 내에서 파일 이름과 링크를 모두 포함하는 첫 번째 <a> 태그를 찾습니다.
                    //     const downloadLink = item.querySelector('a');

                    //     // 미리보기 버튼을 제외하고 유효한 링크인지 확인합니다.
                    //     if (downloadLink && !downloadLink.classList.contains('preview_btn')) {
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.mokpo.go.kr/www/mokpo_news/notification/public_notice',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: '#content table.board_basic tbody tr'
        })
