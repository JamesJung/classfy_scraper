        ...
        while (retries < maxRetries) {
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    // 첨부파일 링크 추출
                    const fileListItems = document.querySelectorAll('.add_file ul li');
                    const attachments = [];

                    fileListItems.forEach(item => {
                        // Find the anchor tag within the current list item.
                        const fileAnchor = item.querySelector('a');

                        if (fileAnchor) {
                            // Get the file name from the text content.
                            const fileName = fileAnchor.textContent.trim();
                            // Get the download URL from the href attribute.
                            const downloadLink = fileAnchor.href;
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.namdong.go.kr/main/eminwon/eminwonAnnounceList.do?pgno=1',
            required: true
        })
        ...
        .option('date-selector', {
            type: 'string',
            description: '날짜 선택자',
            default: '.list_info li:nth-child(2) '
        }).help().argv;
}
