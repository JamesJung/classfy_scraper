...
class PyeongtaekScraper extends AnnouncementScraper {
    constructor(options = {}) {
        const defaultOptions = {
            siteCode: 'pyeongtaek',
            baseUrl: 'https://www.pyeongtaek.go.kr/pyeongtaek/saeol/gosiList.do?seCode=01&mId=0401020100',
            listSelector: 'table tr:not(:first-child)',
            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        'table.p-table', '.bod_view', '.content_box', '.program--contents',
                    ...
                    goDownloadLinks.forEach(link => {
                        ...
                        const downloadClick = link.getAttribute('onclick');
        ...
        try {
            ...
            if (attachment.onclick) {
                ...
                if (goDownloadMatch) {
                    ...
                    downloadUrl = `https://eminwon.pyeongtaek.go.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=${enc_user_file_nm}&sys_file_nm=${enc_sys_file_nm}&file_path=${enc_file_path}`;
    ...
    async buildDetailUrl(announcement) {
        ...
        if (announcement.onclick) {
            ...
            // function boardView(page, idx) {
            //     var frm = document.getElementById('listForm');
            //     frm.page.value = page;
            //     frm.action = '/pyeongtaek/saeol/gosiView.do?notAncmtMgtNo=' + idx + '&mId=0401020100';
            //     frm.submit();
            // }
            const boardViewMatch = announcement.onclick.match(/boardView\s*\(\s*['"]([^'"]+)['"]\s*,\s*['"]([^'"]+)['"]\s*\)/);
            console.log("boardViewMatch", boardViewMatch)
            if (boardViewMatch) {
                const [, page, idx] = boardViewMatch;
                const detailUrl = `https://www.pyeongtaek.go.kr/pyeongtaek/saeol/gosiView.do?notAncmtMgtNo=${idx}&mId=0401020100&page=${page}`;
...
if (require.main === module) {
    ...
    const scraper = new PyeongtaekScraper({
...
module.exports = PyeongtaekScraper;