        ...
        while (retries < maxRetries) {
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    // Select all <a> tags within the <td> of the "파일" row.
                    const fileLinks = document.querySelectorAll('tbody td a');
                    const results = [];
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.chungju.go.kr/www/selectEminwonList.do?key=510&ofr_pageSize=10&ancmt_se_code=01,02,04,05&pageIndex=1',
            required: true
        })
