...
class AnnouncementScraper {
    ...
    async getAnnouncementList(pageNum) {
        ...
        while (retries < maxRetries) {
            try {
                ...
                // 첫 페이지는 URL로 직접 이동, 이후 페이지는 JavaScript로 이동
                if (pageNum === 1) {
                    ...
                    // JavaScript 함수를 사용해 페이지 이동
                    console.log(`페이지 ${pageNum}로 JavaScript 이동`);
                    await this.page.evaluate((pageNumber) => {
                        if (typeof goPage === 'function') {
                            goPage(pageNumber);
                        } else if (typeof fnPage === 'function') {
                            fnPage(pageNumber);
                        } else {
                            // 폼 전송 방식 시도
            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '#contents', '#viewBoardContent', '.bod_view', '.contents_wrap', '.program--contents',
        ...
        try {
            ...
            if (attachment.onclick) {
                ...
                if (goDownloadMatch) {
                    ...
                    downloadUrl = `https://eminwon.siheung.go.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=${enc_user_file_nm}&sys_file_nm=${enc_sys_file_nm}&file_path=${enc_file_path}`;
    ...
    buildListUrl(pageNum) {
        // 시흥시는 JavaScript로 페이지를 이동하므로 URL 파라미터가 필요 없음
        // 첫 페이지만 URL로 접근
        return this.baseUrl;
    }
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.siheung.go.kr/main/saeol/gosi/list.do?mId=0401040100',
            required: true
        })
