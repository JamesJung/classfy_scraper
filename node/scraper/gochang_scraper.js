                ...
                const announcements = await this.page.evaluate((selectors) => {
                    ...
                    rows.forEach((row, index) => {
                        ...
                        if (titleElement && dateElement) {
                            ...
                            const dataAction = linkElement.getAttribute('data-action');
            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '.contents', 'table.skinTb-data-resList', '.program--contents',
                    ...
                    const downloadLinks = document.querySelectorAll('p.bbs_filedown a.ico_file');

                    downloadLinks.forEach(link => {
                        // Get the full text content, which includes the file size in parentheses
                        const fullText = link.textContent.trim();
                        // Use a regular expression to remove the file size
                        const fileName = fullText.replace(/\(\d+\s*kb\)/, '').trim();
    ...
    async downloadSingleAttachment(attachment, attachDir, index) {
        try {
            ...
            if (attachment.onclick) {
                ...
                // goDownload 패턴 처리
                const goDownloadMatch = attachment.onclick.match(/pf_download\(\s*['"]([^'"]*)['"]\s*\)/);


                console.log("goDownloadMatch", goDownloadMatch)
                if (goDownloadMatch) {
                    const link = goDownloadMatch[1];
                    console.log("link", link)
                    // 실제 goDownload 함수와 동일한 URL 패턴 사용
                    downloadUrl = `https://www.jeonnam.go.kr/${link}`;
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.gochang.go.kr/board/list.gochang?boardId=BBS_0000180&menuCd=DOM_000000102003007000&contentsSid=2682&cpath=',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: 'ul.bbs_list > li'
        })
