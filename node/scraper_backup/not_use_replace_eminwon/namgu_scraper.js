...
/**
 * Namgu (Gwangju) 공고 스크래핑 시스템
 * 
 * 기능:
 * 1. 날짜 기반 필터링
 * 2. 리스트 -> 상세 페이지 처리 (searchDetail 함수 사용)
 * 3. iframe 내부 콘텐츠 추출
 * 4. content.md 파일 생성
 * 5. 첨부파일 다운로드
 * 6. 중복 게시물 스킵
...
class NamguScraper extends AnnouncementScraper {
    constructor(options = {}) {
        super({
            ...options,
            baseUrl: options.baseUrl || 'https://www.namgu.gwangju.kr/api/eminwon/gosiList.es?mid=a10604020100&not_ancmt_se_code=01,04',
            listSelector: 'table tbody tr',
            titleSelector: 'td:nth-child(1) a',  // Title is in 1st column  
            dateSelector: 'td:nth-child(3)',   // Date is in 3rd column
            siteCode: 'namgu',
            ...options
        });
    }
    ...
    buildListUrl(pageNum) {
        ...
        // Namgu uses pageIndex parameter
        url.searchParams.set('pageIndex', pageNum);
        return url.toString();
    }

    /**
     * 공고 리스트 가져오기 - Namgu 특화
     */
        ...
        while (retries < maxRetries) {
            try {
                ...
                for (const frame of frames) {
                    ...
                    if (frameUrl.includes('eminwon')) {
                        console.log('iframe 내부에서 데이터 추출 중...');
                        ...
                        announcements = await frame.evaluate(() => {
                            ...
                            rows.forEach((row) => {
                                ...
                                // searchDetail 함수의 파라미터 추출
                                let announcementId = null;
                                if (onclick && onclick.includes('searchDetail')) {
                                    const match = onclick.match(/searchDetail\s*\(\s*['"]?(\d+)['"]?\s*\)/);
                                    if (match) {
                                        announcementId = match[1];
                                    }
                                }
                        ...
                        if (announcements.length > 0) {
                            break; // Found data in iframe, stop searching
                        }
                    }
                }
                ...
                if (announcements.length === 0) {
                    console.log('메인 페이지에서 데이터 추출 중...');

                    // First check how many rows exist
                    const rowCount = await this.page.evaluate(() => {
                        return document.querySelectorAll('tbody tr').length;
                    });
                    console.log(`Found ${rowCount} tbody tr elements`);

                    announcements = await this.page.evaluate(() => {
                        ...
                        rows.forEach((row, idx) => {
                            ...
                            if (hasTh || !hasTd) {
                                console.log(`Row ${idx}: Skipped (has th: ${hasTh}, has td: ${hasTd})`);
                                return;
                            }

                            const cells = row.querySelectorAll('td');
                            if (cells.length < 4) {
                                console.log(`Row ${idx}: Skipped (only ${cells.length} cells)`);
                                return;
                            }
    ...
    async getDetailContent(announcement) {
        ...
        while (retries < maxRetries) {
            try {
                ...
                // searchDetail 함수 호출하여 상세 페이지로 이동
                await this.page.evaluate((id) => {
                    // searchDetail 함수가 있으면 호출
                    if (typeof searchDetail === 'function') {
                        searchDetail(id);
                    } else {
                        // iframe 내부에서 호출 시도
                        const iframe = document.querySelector('iframe');
                        if (iframe && iframe.contentWindow && iframe.contentWindow.searchDetail) {
                            iframe.contentWindow.searchDetail(id);
                        }
                    }
                ...
                // iframe이나 새 창에서 내용 추출
                let content = null;

                // 먼저 iframe에서 상세 내용 찾기
                const frames = this.page.frames();
                for (const frame of frames) {
                    const frameUrl = frame.url();
                    if (frameUrl.includes('eminwon') || frameUrl.includes('gosiView')) {
                        try {
                            content = await frame.evaluate(() => {
                                ...
                                for (const row of rows) {
                                    ...
                                    if (th && (th.textContent.includes('게시일') || th.textContent.includes('등록일'))) {
                ...
                if (!content || !content.content) {
                    content = await this.page.evaluate(() => {
                        ...
                        // 첨부파일 추출
                        const attachments = [];
                        const fileLinks = document.querySelectorAll('a[href*="download"], a[onclick*="download"]');
    ...
    /**
     * 상세 페이지 URL 구성 - Namgu는 JavaScript 함수 사용
     */
    async buildDetailUrl(announcement) {
        // Namgu는 searchDetail 함수를 사용하므로 URL 구성 불필요
        return null;
...
if (require.main === module) {
    ...
    const scraper = new NamguScraper({
