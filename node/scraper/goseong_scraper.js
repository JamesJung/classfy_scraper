        ...
        while (retries < maxRetries) {
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const listItems = document.querySelectorAll('.bdvFileList');

                    listItems.forEach(item => {
                        // 현재 목록 항목에서 첫 번째 <a> 태그를 찾습니다.
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.goseong.go.kr/board/list.goseong?boardId=BBS_0000015&menuCd=DOM_000000103001014000&contentsSid=29&cpath=',
            required: true
        })
