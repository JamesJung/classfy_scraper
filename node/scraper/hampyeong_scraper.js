        ...
        while (retries < maxRetries) {
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const fileItems = document.querySelectorAll('table td.td_left');

                    fileItems.forEach(item => {
                        // 다운로드 링크가 포함된 <a> 태그를 찾습니다.
                        const downloadLink = item.querySelector('a[onclick*="goDown"]');
                        if (downloadLink) {
                            // onclick 속성 값을 가져옵니다.
                            const onclickAttribute = downloadLink.getAttribute('onclick');

                            // 정규표현식을 사용하여 goDownLoad() 함수의 인자들을 추출합니다.
                            const regex = /goDown\('([^']+)',\s*'([^']+)',\s*'([^']+)'\)/;
                            ...
                            if (matches && matches.length === 4) {
                                ...
                                const fullUrl = `https://eminwon.hampyeong.go.kr/emwp/jsp/ofr/FileDownNew.jsp?user_file_nm=${enc_user_file_nm}&sys_file_nm=${enc_sys_file_nm}&file_path=${enc_file_path}`;
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.hampyeong.go.kr/pg/GosiList.do?pageId=www273',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: 'div.board_list_body .body_row'
        })
        .option('title-selector', {
            type: 'string',
            description: '제목 선택자',
            default: 'div.subject a'
        })
