...
class AnnouncementScraper {
    constructor(options = {}) {
        ...
        this.siteCode = options.siteCode || 'nyj';
        this.outputDir = path.join(this.baseOutputDir, this.siteCode); // 사이트별 폴더 생성
        this.baseUrl = options.baseUrl || 'https://www.nyj.go.kr/www/selectEminwonWebList.do?key=2492';
        this.listSelector = options.listSelector || 'tbody tr';
        this.titleSelector = options.titleSelector || 'td:nth-child(2) a';
        this.dateSelector = options.dateSelector || 'td:nth-child(4)';
        this.browser = null;
        this.page = null;
    ...
    async initBrowser() {
        ...
        while (retries < maxRetries) {
            try {
                // Playwright로 브라우저 실행 - 개발자 도구 감지 우회
                this.browser = await chromium.launch({
                    headless: false,  // headless 비활성화
                    args: [
    ...
    async getAnnouncementList(pageNum) {
        ...
        while (retries < maxRetries) {
            try {
                ...
                // 디버깅용 스크린샷
                await this.page.screenshot({ path: `debug_nyj_page${pageNum}.png` });
                ...
                const hasTable = await this.page.evaluate(() => {
                    ...
                    return { hasTable: !!tbody, rowCount: rows.length };
                });
                console.log('테이블 확인:', hasTable);
        ...
        try {
            ...
            if (attachment.onclick) {
                ...
                if (goDownloadMatch) {
                    ...
                    downloadUrl = `https://eminwon.cheongdo.go.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=${enc_user_file_nm}&sys_file_nm=${enc_sys_file_nm}&file_path=${enc_file_path}`;
    ...
    buildListUrl(pageNum) {
        // 남양주시는 pageIndex 파라미터 사용
        const url = new URL('https://www.nyj.go.kr/www/selectEminwonWebList.do');
        url.searchParams.set('key', '2492');
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.nyj.go.kr/www/selectEminwonWebList.do?key=2492&sa1=01&sa1=02&sa1=04&sa1=05&sc4=2024',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: '#contents table.p-table tbody tr'
        })
