        ...
        while (retries < maxRetries) {
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const attachmentItems = document.querySelectorAll('.ui.bbs--view--file > li');

                    attachmentItems.forEach(item => {
                        const linkElement = item.querySelector('a[href*="mode=D"]');

                        if (linkElement) {
                            const fileNameElement = linkElement.getAttribute('title');
                            const downloadLinkElement = linkElement.href;
                            const url = linkElement.href;

                            if (fileNameElement && downloadLinkElement) {
                                // 파일 이름에서 이미지 태그와 alt 텍스트를 제외하고 텍스트만 가져오기
                                const fileName = fileNameElement.trim();
                                const href = downloadLinkElement;
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.yd21.go.kr/kr/html/sub02/020103.html?GotoPage=1',
            required: true
        })
