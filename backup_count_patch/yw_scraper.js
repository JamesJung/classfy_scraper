        ...
        while (retries < maxRetries) {
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const fileItems = document.querySelectorAll('ul.view_attach li');
                    const results = [];

                    fileItems.forEach(item => {
                        // 파일 이름을 포함하는 <span> 태그를 찾아 텍스트를 추출합니다.
                        const fileNameElement = item.querySelector('.down_view span');
                        const fileName = fileNameElement ? fileNameElement.textContent.trim() : '';

                        // 다운로드 링크를 포함하는 <a> 태그를 찾아 href 속성을 추출합니다.
                        const downloadLink = item.querySelector('a.file_down');
                        const url = downloadLink ? downloadLink.href : '';
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.yw.go.kr/www/selectBbsNttList.do?bbsNo=17&key=273',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: '#contents table.bbs_default tbody tr'
        })
