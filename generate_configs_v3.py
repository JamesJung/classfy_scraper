#!/usr/bin/env python3
"""
모든 스크래퍼 파일을 분석하여 config JSON 파일 생성 (v3 - 주석 처리된 부분도 고려)
"""
...
def extract_scraper_info(filepath):
    ...
    # 주석 처리된 default 값도 체크
    if not list_selector_match:
        # 주석 처리된 라인의 패턴 확인
        list_selector_comment = re.search(
            r"\.option\(['\"]list-selector['\"][^}]*?//\s*default:\s*['\"`]([^'\"`]+)['\"`]",
            content, re.DOTALL
        )
        if list_selector_comment:
            # 주석 다음 라인에서 실제 default 찾기
            list_selector_real = re.search(
                r"\.option\(['\"]list-selector['\"][^}]*?//[^\n]*\n\s*default:\s*['\"`]([^'\"`]+)['\"`]",
                content, re.DOTALL
            )
            if list_selector_real:
                list_selector_match = list_selector_real
    ...
    if build_detail_url_match:
        ...
        if 'boardView' in func_body:
            # boardView 패턴 검색
            board_pattern_match = re.search(r'boardView.*?match\(([^)]+)\)', func_body)
            if board_pattern_match:
                # URL 템플릿 찾기
        ...
        special_params = ['notAncmtMgtNo', 'mgtNo', 'boardSeq', 'bbsSeq', 'articleNo', 'nttId', 'bbsNttSeq']
...
def main():
    ...
    # 테스트할 몇 개 파일만 먼저
    test_files = ['anseong_scraper.js', 'cs_scraper.js', 'cwg_scraper.js', 'pohang_scraper.js', 'chilgok_scraper.js']
    
    for test_file in test_files:
        scraper_file = scraper_dir / test_file
        ...
        # 결과 출력
        print(f"사이트: {info['siteName']} ({info['siteCode']})")
        print(f"URL: {info['listUrl']}")
        print(f"셀렉터:")
        print(f"  - list: {info['selectors'].get('list', 'N/A')}")
        print(f"  - row: {info['selectors'].get('row', 'N/A')}")
        print(f"  - link: {info['selectors'].get('link', 'N/A')}")
        print(f"  - date: {info['selectors'].get('date', 'N/A')}")
        print(f"페이지네이션: {info['pagination']}")
        print(f"onclick 패턴: {len(info['onclickPatterns'])}개")
