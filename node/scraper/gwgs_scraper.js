            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '#contents', '.content', '.program--view', '.main', '.article', '.post',
                        '#main', '#article', '#post',
                    ...
                    if (!dateText) {
                        ...
                        if (announcement && announcement.listDate) {
                            ...
                            const dateElements = document.querySelectorAll('#pbsHopYmd');
                            if (dateElements) {
                                dateText = dateElements.textContent.trim();
                            }
                        }
                    ...
                    // // 두 NodeList를 합치기
                    // const allLinks = [...attachmentLinks];
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.gwgs.go.kr/prog/saeolGosi/GOSI/kor/sub04_030401/list.do?pageIndex=1',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: 'table.bbsTable tbody tr'
        })
