...
class AnnouncementScraper {
            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '#content', '.bbs__view', '.contents_wrap', '.program--contents',
                        '.bbs_skin', '.view_box',
                    ...
                    const fileSelectors = document.querySelectorAll('.p-attach .p-attach__item',);

                    fileSelectors.forEach(selector => {

                        console.log("selector", selector)
                        const fileNameElement = selector.querySelector("span");
                        const linkElement = selector.querySelector("a.p-attach__down");
    ...
    async downloadViaEgovPost(fileNm, sysFileNm, filePath, attachDir, fileName) {
        try {
                ...
                const downloadHandler = async (download) => {
                    try {
                        ...
                        resolve({
                            success: true,
                            savedPath: savePath,
                            actualDownloadUrl: `https://eminwon.hanam.go.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=${encodeURIComponent(fileNm)}&sys_file_nm=${encodeURIComponent(sysFileNm)}&file_path=${encodeURIComponent(filePath)}`
                        });
            ...
            try {
                ...
                return {
                    ...downloadResult,
                    actualDownloadUrl: downloadResult.actualDownloadUrl || `https://eminwon.hanam.go.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=${encodeURIComponent(fileNm)}&sys_file_nm=${encodeURIComponent(sysFileNm)}&file_path=${encodeURIComponent(filePath)}`
                };
            } catch (downloadError) {
    ...
    async downloadViaAxiosWithSession(url, userFileNm, sysFileNm, filePath, attachDir, displayFileName, cookieString, referer) {
        try {
            ...
            const downloadUrl = url || 'https://eminwon.hanam.go.kr/emwp/jsp/ofr/FileDown.jsp';
    ...
    generateMarkdownContent(announcement, detailContent) {
        ...
        if (detailContent.attachments && detailContent.attachments.length > 0) {
            ...
            detailContent.attachments.forEach((att, i) => {
                ...
                } else if (att.url && att.url.includes('gourl')) {
                    // gourl 패턴에서 실제 다운로드 URL 생성
                    const regex = /gourl\('([^']+)',\s*'([^']+)',\s*'([^']+)'\)/;
                    const matches = att.url.match(regex);
                    if (matches) {
                        const [, userFileNm, sysFileNm, filePath] = matches;
                        const actualUrl = `https://eminwon.hanam.go.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=${encodeURIComponent(userFileNm)}&sys_file_nm=${encodeURIComponent(sysFileNm)}&file_path=${encodeURIComponent(filePath)}`;
                        attachInfo = `${i + 1}. ${att.name}: ${actualUrl}`
                    } else {
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.hanam.go.kr/www/selectGosiList.do?key=171&not_ancmt_se_code=01,04',
            required: true
        })
