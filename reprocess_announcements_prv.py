#!/usr/bin/env python3
"""
PRV 공고 재처리 스크립트 (날짜 필터링 지원)

특정 ID의 PRV 공고들을 다시 Ollama로 분석하여 지원내용과 지원대상을 재추출합니다.
날짜 필터링 기능으로 특정 날짜 이전의 공고만 처리할 수 있습니다.

사용법:
    python reprocess_announcements_prv.py 1,2,3,4,5
    python reprocess_announcements_prv.py 1,2,3,4,5 --date 20250710

파라미터:
    ids: 재처리할 PRV 레코드 ID들 (콤마로 구분)
    --date: 작성일 필터링 (YYYYMMDD 형식, 해당 날짜 이전 공고만 처리)
"""
