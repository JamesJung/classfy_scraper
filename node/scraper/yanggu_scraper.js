...
class AnnouncementScraper {
            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '#contents', 'tbody.p-table--th-left', '.bbs__view', '.contents_wrap', '.program--contents',
                        '.board_view', '#user_board_whole',
                    ];
                    ...
                    // 첨부파일 링크 추출
                    const attachments = [];
                    const addedFiles = new Set(); // 중복 제거용

                    // 양구군은 javascript:opendownload 형식 사용
                    const fileLinks = document.querySelectorAll('a[href*="opendownload"]');

                    fileLinks.forEach(link => {
                        const href = link.href;
                        const fileName = link.textContent.trim();

                        // "다운로드" 버튼은 제외, 실제 파일명이 있는 링크만 처리
                        if (fileName && !fileName.includes('다운로드') && !fileName.includes('미리보기')) {
                            // 중복 제거
                            if (!addedFiles.has(fileName)) {
                                ...
                                console.log('첨부파일 발견:', fileName, href);
                            }
                        }
                    });
    ...
    async downloadSingleAttachment(attachment, attachDir, index) {
        try {
            ...
            // javascript:opendownload('announcement', 144946, 1) 패턴 처리
            if (downloadUrl && downloadUrl.includes('opendownload')) {
                const opendownloadMatch = downloadUrl.match(/opendownload\(\s*['"]([^'"]+)['"]\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/);

                if (opendownloadMatch) {
                    const [, bcd, bn, num] = opendownloadMatch;
                    // 양구군 실제 다운로드 URL 생성 (fnc_bbs/user_bbs_download 사용)
                    downloadUrl = `https://www.yanggu.go.kr/fnc_bbs/user_bbs_download?bcd=${bcd}&bn=${bn}&num=${num}`;
                    console.log('opendownload 패턴 변환:', downloadUrl);
                }
            }

            // URL에서 goDownload 패턴 확인 (다른 페이지용)
            else if (downloadUrl && downloadUrl.includes('goDownload')) {
                // goDownload 함수 호출 패턴 분석
                const goDownloadMatch = downloadUrl.match(/goDownload\(\s*['"]([^'"]*)['"]\s*,\s*['"]([^'"]*)['"]\s*,\s*['"]([^'"]*)['"]\s*\)/);
                ...
                if (goDownloadMatch) {
                    ...
                    downloadUrl = `https://eminwon.yanggu.go.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=${enc_user_file_nm}&sys_file_nm=${enc_sys_file_nm}&file_path=${enc_file_path}`;
                    fileName = originalName; // 원본 파일명 사용
                    console.log('goDownload 패턴 변환:', downloadUrl);
                }
            }

            // JavaScript 방식 처리 (onclick이 있는 경우)
    ...
    generateMarkdownContent(announcement, detailContent) {
        ...
        if (detailContent.attachments && detailContent.attachments.length > 0) {
            ...
            detailContent.attachments.forEach((att, i) => {
                ...
                // javascript:opendownload 패턴을 실제 다운로드 URL로 변환
                if (downloadUrl && downloadUrl.includes('opendownload')) {
                    const opendownloadMatch = downloadUrl.match(/opendownload\(\s*['"]([^'"]+)['"]\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/);
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.yanggu.go.kr/user_sub?gfnc=www&mu_idx=226',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: '#user_board_list table tbody tr'
        })
