...
class AnnouncementScraper {
    ...
    async getAnnouncementList(pageNum) {
        ...
        while (retries < maxRetries) {
            try {
                ...
                console.log('AJAX 데이터 로딩 대기 중...');
                
                // AJAX POST 요청 완료 대기
                await this.page.waitForResponse(
                    response => response.url().includes('eminwon.gijang.go.kr/emwp/gov/mogaha/ntis/web/ofr/action/OfrAction.do') 
                                && response.status() === 200,
                    { timeout: 10000 }
                ).catch(() => {
                    console.log('AJAX 응답 대기 시간 초과, 계속 진행...');
                });
                ...
                // 테이블 행이 로드될 때까지 대기
                try {
                    await this.page.waitForSelector('table tbody tr td', { timeout: 5000 });
                    console.log('테이블 데이터 로드 완료');
                } catch (e) {
                    console.log('테이블 데이터 대기 시간 초과');
                }
                ...
                for (const frame of frames) {
                    ...
                    // eminwon 프레임 찾기
                    if (frameUrl.includes('eminwon.gijang.go.kr')) {
                        console.log(`eminwon 프레임에서 데이터 추출: ${frameUrl}`);
                        
                        try {
                            announcements = await frame.evaluate(() => {
                                ...
                                rows.forEach((row, index) => {
                                    ...
                                    if (index === 0) return; // 첫 번째 행도 스킵 (보통 헤더)
                                    
                                    const cells = row.querySelectorAll('td');
                                    if (cells.length >= 5) {
                                        // 기장군 구조: 번호(0), 공고번호(1), 제목(2), 부서(3), 날짜(4)
                                        const titleCell = cells[2]; // 제목은 3번째 셀
                                        const titleElement = titleCell ? titleCell.querySelector('a') : null;
                                        const dateCell = cells[4]; // 날짜는 5번째 셀
                                        const noticeNoCell = cells[1]; // 공고번호는 2번째 셀
                                        ...
                                        if (titleElement) {
                                            ...
                                            const noticeNo = noticeNoCell ? noticeNoCell.textContent.trim() : '';
                                            
                                            // 제목과 공고번호 결합
                                            const fullTitle = noticeNo ? `${noticeNo} ${title}` : title;
                            ...
                            if (announcements.length > 0) {
                                console.log(`iframe에서 ${announcements.length}개 공고 발견`);
                                break; // 데이터를 찾았으면 종료
                            }
                        } catch (frameError) {
                            console.log(`프레임 데이터 추출 실패: ${frameError.message}`);
                        }
                    }
                }
                
                // iframe에서 데이터를 못 찾은 경우 메인 페이지에서 시도
                if (announcements.length === 0) {
                    console.log('iframe에서 데이터를 찾지 못해 메인 페이지에서 시도...');
                    announcements = await this.page.evaluate((selectors) => {
                        ...
                        rows.forEach((row, index) => {
                            ...
                            if (titleElement) {
                                const title = titleElement.textContent.trim();
                                const dateText = dateElement ? dateElement.textContent.replace("등록일:", '').trim() : '';
                                // 다양한 방식으로 링크 정보 추출
    ...
    async buildDetailUrl(announcement) {
        ...
        if (announcement.onclick) {
            ...
            // searchDetail 패턴 처리 (기장군 특유 패턴)
            const searchDetailMatch = announcement.onclick.match(/searchDetail\(['"](\d+)['"]\)/);
            if (searchDetailMatch) {
                const noticeNo = searchDetailMatch[1];
                // eminwon 시스템 상세페이지 URL 구성 - method 파라미터 추가
                const detailUrl = `https://eminwon.gijang.go.kr/emwp/gov/mogaha/ntis/web/ofr/action/OfrAction.do?method=selectOfr&not_ancmt_mgt_no=${noticeNo}&not_ancmt_se_code=01,02,04&title=`;
                console.log('searchDetail 패턴으로 구성된 URL:', detailUrl);
                return detailUrl;
            }
            
            // goNoticeView 패턴 처리 (기장군 특유 패턴)
            const goNoticeViewMatch = announcement.onclick.match(/goNoticeView\(([^)]+)\)/);
            if (goNoticeViewMatch) {
                const params = goNoticeViewMatch[1].split(',').map(p => p.trim().replace(/['"]/g, ''));
                // 보통 goNoticeView('고시번호', '기타파라미터') 형태
                if (params.length > 0) {
                    const noticeNo = params[0];
                    // eminwon 시스템 상세페이지 URL 구성
                    const detailUrl = `https://eminwon.gijang.go.kr/emwp/gov/mogaha/ntis/web/ofr/action/OfrAction.do?not_ancmt_mgt_no=${noticeNo}`;
                    console.log('goNoticeView 패턴으로 구성된 URL:', detailUrl);
                    return detailUrl;
                }
            }
    ...
    buildListUrl(pageNum) {
        ...
        url.searchParams.set('searchPageIndex', pageNum);
        return url.toString();
    }
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.gijang.go.kr/index.gijang?menuCd=DOM_000000101001002000',
            required: true
        })
