...
class AnnouncementScraper {
    ...
    async getDetailContent(announcement) {
        ...
        while (retries < maxRetries) {
            try {
                ...
                // buildDetailUrl이 이미 상세 페이지로 네비게이션 했는지 확인
                ...
                // goView 패턴이 아닌 경우에만 페이지 이동
                // (goView는 이미 buildDetailUrl에서 form submit으로 이동함)
                if (!announcement.link || !announcement.link.includes('goView')) {
                    // 상세 페이지로 이동
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        'table.p-table', '#sub_content', '.content_box', '.program--contents',
                    ...
                    const downloadLinks = document.querySelectorAll('a[title="다운로드"]');
    ...
    async downloadSingleAttachment(attachment, attachDir, index) {
        try {
            ...
            // 서대문구 특별 처리 - URL에서 파라미터 추출
            if (downloadUrl && downloadUrl.includes('/common/board/download.do')) {
                // URL 파싱하여 파라미터 추출
                const urlObj = new URL(downloadUrl, this.baseUrl);
                const params = urlObj.searchParams;
                
                // 서대문구 다운로드 URL은 그대로 사용
                downloadUrl = urlObj.toString();
                downloadType = 'sdm_download';
                
                console.log(`서대문구 다운로드 URL: ${downloadUrl}`);
            }
            // JavaScript 방식 처리
                ...
                if (goDownloadMatch) {
                    ...
                    // 서대문구 goDownload 함수와 동일한 URL 패턴 사용
                    ...
                    downloadUrl = `https://eminwon.sdm.go.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=${enc_user_file_nm}&sys_file_nm=${enc_sys_file_nm}&file_path=${enc_file_path}`;
    ...
    async downloadViaLink(url, attachDir, fileName) {
        try {
            ...
            const headers = {
                ...
                'Referer': 'https://www.sdm.go.kr/news/notice/notice.do'
            };
    ...
    async buildDetailUrl(announcement) {
        ...
        if (announcement.link) {
            ...
            const viewMatch = announcement.link.match(/goView\(\s*['"]([^'"]*)['"]\s*\)/);

            console.log(viewMatch)

            if (viewMatch) {
                const keyValue = viewMatch[1];

                // Form submit을 통한 네비게이션 처리
                await Promise.all([
                    ...
                    this.page.evaluate((params) => {
                        ...
                        form.id = 'frm';
                        form.method = 'post';
                        form.action = 'https://www.sdm.go.kr/news/notice/notice.do';
                        form.target = '_self';
                        ...
                        const inputs = [
                            { name: 'sdmBoardSeq', value: keyValue },
                            { name: 'mode', value: "view" },
                            { name: 'sdmBoardConfSeq', value: "82" }
                        ];
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.sdm.go.kr/news/notice/notice.do',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: 'table.boardList tbody tr'
