...
class AnnouncementScraper {
                ...
                const announcements = await this.page.evaluate((selectors) => {
                    ...
                    rows.forEach((row, index) => {
                        ...
                        if (titleElement && dateElement) {
                            const title = titleElement.textContent.trim().replace("NEW", "");
            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '#contents', '#board', '.content', 'table.p-table', '.program--contents',
                    ...
                    // 첨부파일 링크 추출
                    const downloadLinks = document.querySelectorAll('a[href^="javascript:gourl"]');
                    const attachments = [];

                    downloadLinks.forEach(link => {
                        // Get the file name from the text content.
                        const fileName = link.textContent.trim();

                        // The link is embedded in the href attribute.
                        const downloadLink = link.href.trim();
    ...
    async downloadSingleAttachment(attachment, attachDir, index) {
        try {
            ...
            // 송파구 특별 처리: javascript:gourl() 형식에서 URL 추출
            console.log('원본 downloadUrl:', downloadUrl);
            if (downloadUrl && downloadUrl.includes('javascript:gourl')) {
                // javascript:gourl('...'%27); 형식에서 URL 추출
                // gourl(' 다음부터 마지막 %27); 이전까지
                const startIdx = downloadUrl.indexOf("('") + 2;
                const endIdx = downloadUrl.lastIndexOf("%27);");

                if (startIdx > 1 && endIdx > startIdx) {
                    downloadUrl = downloadUrl.substring(startIdx, endIdx);
                    downloadType = 'songpa_gourl';
                    console.log('송파구 gourl URL 추출:', downloadUrl);
                    actualDownloadUrl = downloadUrl; // 추출한 URL을 실제 다운로드 URL로 설정
                }
            }
            // JavaScript 방식 처리
    ...
    async downloadViaLink(url, attachDir, fileName) {
        try {
            ...
            const headers = {
                ...
                'Referer': 'https://www.songpa.go.kr/'
            };

            // 송파구 eminwon.seoul.kr 도메인인 경우 특별 처리
            if (url.includes('songpa.eminwon.seoul.kr')) {
                headers['Content-Length'] = '0';  // 빈 Content-Length 헤더 추가
            }
            ...
            const response = await axios({
                ...
                validateStatus: function (status) {
                    return status >= 200 && status < 500; // 4xx 에러도 처리 가능하도록
                }
            });
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.songpa.go.kr/www/selectGosiList.do?key=2776&',
            required: true
        })
