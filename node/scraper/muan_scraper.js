        ...
        while (retries < maxRetries) {
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const fileItems = document.querySelectorAll('.attach_thum li a');
                    ...
                    fileItems.forEach(link => {
                        ...
                        if (matches && matches.length === 4) {
                            ...
                            const fullUrl = `https://eminwon.muan.go.kr/emwp/jsp/ofr/FileDownNew.jsp?user_file_nm=${encodeURIComponent(fileName)}&sys_file_nm=${encodeURIComponent(systemFileName)}&file_path=${encodeURIComponent(filePath)}`;
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.muan.go.kr/www/openmuan/new/announcement',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: '#content table.board_t1 tbody tr'
        })
