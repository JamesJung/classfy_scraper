...
class AnnouncementScraper {
    ...
    async getAnnouncementList(pageNum) {
        ...
        while (retries < maxRetries) {
            try {
                ...
                // 경기도 사이트는 URL 앵커(#page=n) 방식으로 페이지네이션 처리
                if (pageNum === 1) {
                    ...
                    // 테이블이 로드될 때까지 대기
                    await this.page.waitForSelector('#boardList', { timeout: 10000 });
                } else {
                    // 2페이지 이상은 URL 앵커를 변경하여 이동
                    console.log(`페이지 ${pageNum}로 이동 시도...`);
                    
                    // 방법 1: URL 직접 이동 (앵커 방식)
                    ...
                    // 방법 2: gotoPage 함수가 있다면 호출
                    const gotoPageSuccess = await this.page.evaluate((targetPage) => {
                    ...
                    if (gotoPageSuccess) {
                        console.log(`gotoPage 함수로 페이지 ${pageNum} 이동`);
                        await this.page.waitForTimeout(2000);
                    }
                    
                    // 테이블 재로드 대기
                    try {
                        await this.page.waitForSelector('#boardList', { timeout: 5000 });
                    } catch (e) {
                    ...
                    const currentPage = await this.page.evaluate(() => {
                        const activePageLink = document.querySelector('.pagination .page-nav-item.active, .pagination li.active');
                        return activePageLink ? activePageLink.textContent.trim() : '알 수 없음';
                    });
                    console.log(`현재 활성 페이지: ${currentPage}`);
                }
                ...
                await this.page.waitForSelector('#boardList tbody tr:nth-child(5)', { 
                    timeout: 10000,
                    state: 'visible' 
                }).catch(error => {
                    console.log('테이블 행 대기 실패:', error.message);
                });

                // 리스트 요소들 추출
                const announcements = await this.page.evaluate((selectors) => {
                    // 셀렉터로 행 찾기
                    const rows = document.querySelectorAll(selectors.listSelector || '#boardList tbody tr');
                    
                    // rows를 배열로 변환 (NodeList 또는 HTMLCollection 모두 지원)
                    const rowsArray = Array.from(rows);
                    
                    const titleSelector = selectors.titleSelector || 'td:nth-child(2) a';
                    const dateSelector = selectors.dateSelector || 'td:nth-child(5)';
                    
                    const results = [];

                    rowsArray.forEach((row, index) => {
                        // 헤더 행 스킵 (gg.go.kr는 헤더가 thead에 있음)
            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '.s-v-board-default', '.contents_wrap', '.program--contents',
                    ...
                    const fileList = document.querySelector('dl.file dd ul');
                    const items = fileList.querySelectorAll('li');

                    items.forEach(item => {
                        // Select the <a> tag that contains the file name and the "다운로드" title
                        const downloadLink = item.querySelector('a.filename[title="다운로드"]');

                        if (downloadLink) {
                            const fileName = downloadLink.textContent.trim();
                            const fileURL = downloadLink.href;

                            const match = fileName.match(/^(.+?)\s*\[\d+KB\]/);

                            let fileNameExtract = '';
                            if (match && match.length > 1) {
                                // The captured group at index 1 contains the file name without the size
                                fileNameExtract = match[1].trim();
                            } else {
                                // If the string doesn't have the expected format, use the original string
                            ...
                            attachments.push(
                                { name: fileNameExtract, url: fileURL }
                            );
    ...
    buildListUrl(pageNum) {
        // gg.go.kr는 첫 페이지는 hash 없이, 이후 페이지는 gotoPage 함수 사용
        const baseUrlWithoutHash = this.baseUrl.split('#')[0];
        
        // 첫 페이지는 hash 없이 로드해야 정상적으로 테이블이 로드됨
        if (pageNum === 1) {
            return baseUrlWithoutHash;
        }
        
        // 2페이지 이상은 첫 페이지 로드 후 gotoPage로 이동
        return baseUrlWithoutHash;
    }
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.gg.go.kr/bbs/board.do?bsIdx=469&menuId=1547#page=1',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: '#boardList > tbody > tr'
        })
