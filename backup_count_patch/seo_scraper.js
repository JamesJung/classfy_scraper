        ...
        while (retries < maxRetries) {
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const listItems = document.querySelectorAll('dd ul li');
                    const attachments = [];

                    listItems.forEach(item => {
                        // 다운로드 링크를 가진 <a> 태그를 찾습니다.
                        const downloadLinkElement = item.querySelector('a[href*="FileDown.do"]');
                        // 파일 크기를 가진 <span> 태그를 찾습니다.
                        const fileSizeElement = item.querySelector('.wfont');
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.seo.incheon.kr/open_content/main/community/news/gosi.jsp',
            required: true
        })
