...
eminwon_data_new와 scraped_incremental_v2 폴더의 데이터를
...
class UnifiedBatchProcessor:
    ...
    def setup_logging(self):
        ...
        log_file = log_dir / f'{self.data_source}_batch_{self.target_date}.log'
    ...
    def process_region(self, region_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            cmd = [
                ...
                '-d', str(self.base_dir),  # 부모 디렉토리
                '--site-code', normalized_site_code  # 정규화된 site_code 사용
            ]
    ...
    def run(self):
        ...
        self.logger.info('='*80)
        self.logger.info(f"데이터 소스: {self.data_source.upper()}")
    ...
    def save_results(self, results: List[Dict]):
        """처리 결과 저장"""
        result_file = Path('logs') / f'{self.data_source}_batch_results_{self.target_date}.json'
