...
class AnnouncementScraper {
        ...
        while (retries < maxRetries) {
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    // 본문 추출 시도 (gc.go.kr용 selector 추가)
                    let mainContent = null;
                    const contentSelectors = [
                        '.view_detail', '.view_content', '.bbs_content', '.board_content',
                    ...
                    if (!dateText) {
                        ...
                        if (announcement && announcement.listDate) {
                            ...
                            // gc.go.kr 날짜 처리
                            const datePatterns = [
                                '.view_info span:contains("등록일")',
                                '[class*="date"]:contains("등록일")',
                                'td:contains("등록일") + td',
                                '.p-author__info .p-split'
                            ];

                            for (const pattern of datePatterns) {
                                try {
                                    const dateElement = document.querySelector(pattern);
                                    if (dateElement) {
                                        dateText = dateElement.textContent.replace(/등록일|작성일|:/g, '').trim();
                                        break;
                                    }
                                } catch (e) {
                                    // selector 오류 무시
                    ...
                    // gc.go.kr 사이트의 첨부파일 처리
                    const fileLinks = document.querySelectorAll('.file_down_view a, .file_list a, .attach a, [class*="file"] a');
                    ...
                    fileLinks.forEach(link => {
                        ...
                        if (fileName && (href || onclick)) {
                            // "바로 보기" 텍스트 제거
                            const cleanName = fileName.replace(/바로\s*보기/g, '').trim();
    ...
    async downloadSingleAttachment(attachment, attachDir, index) {
        try {
            ...
            // gc.go.kr 사이트인 경우 특별 처리
            if (this.siteCode === 'gc' && (attachment.onclick || attachment.url)) {
                const result = await this.downloadViaGc(attachment, attachDir, fileName);
                if (result) {
     ...
     * gc.go.kr 전용 다운로드 처리
     */
    async downloadViaGc(attachment, attachDir, fileName) {
        try {
            let downloadUrl = attachment.url;

            // onclick에서 goDownload 함수 호출 처리
            if (attachment.onclick && attachment.onclick.includes('goDownload')) {
                // goDownload 함수 호출에서 파라미터 추출
                // goDownload('file.hwp', 'sys123.hwp', '/path/to/file') 형식
                const match = attachment.onclick.match(/goDownload\s*\(\s*['"]([^'"]*)['"]\s*,\s*['"]([^'"]*)['"]\s*,\s*['"]([^'"]*)['"]\s*\)/);
                if (match) {
                    const [, userFileName, sysFileName, filePath] = match;
                    // gc.go.kr의 실제 다운로드 URL 패턴 (eminwon.gc.go.kr 사용)
                    const enc_user_file_nm = encodeURIComponent(userFileName);
                    const enc_sys_file_nm = encodeURIComponent(sysFileName);
                    const enc_file_path = encodeURIComponent(filePath);
                    downloadUrl = `https://eminwon.gc.go.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=${enc_user_file_nm}&sys_file_nm=${enc_sys_file_nm}&file_path=${enc_file_path}`;
                    fileName = userFileName; // 원본 파일명 사용
                }
            }
            ...
            console.error(`gc.go.kr 첨부파일 다운로드 실패:`, error);
            return null;
        }
    }
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.gc.go.kr/portal/saeol/gosi/list.do?seCode=01&mId=1202180100',
            required: true
        })
