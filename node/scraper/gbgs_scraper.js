    ...
    async downloadSingleAttachment(attachment, attachDir, index) {
        try {
            ...
            if (attachment.onclick) {
                // goDownload 패턴 처리
                const goDownloadMatch = attachment.onclick.match(/openDownloadFiles\(\s*['"]([^'"]*)['"]\s*\)/);
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.gbgs.go.kr/open_content/ko/page.do?mnu_uid=2160&',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: '#content table.table_st02  tbody tr'
        })
