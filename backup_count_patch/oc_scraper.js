        ...
        while (retries < maxRetries) {
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const attachmentItems = document.querySelectorAll('a.p-attach__link');

                    attachmentItems.forEach(item => {
                        const linkElement = item.querySelector('span:not(.p-icon)');
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.oc.go.kr/www/selectBbsNttList.do?key=236&bbsNo=40&searchCtgry=&searchCnd=all&searchKrwd=&integrDeptCode=&pageIndex=2',
            required: true
        })
