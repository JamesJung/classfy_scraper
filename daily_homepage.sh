...
# =============================================================================
# Homepage 일일 수집 스크립트 (날짜 기반 버전 v1.0)
# homepage_site_announcement_date 테이블 기반 날짜별 증분 수집
# 단계별 로깅 및 문제 진단 기능 추가
# =============================================================================

# ============= 0. 즉시 로깅 시작 (임시 로그파일) =============
TEMP_LOG="/tmp/homepage_startup_$(date +%Y%m%d_%H%M%S).log"
...
LOG_FILE="${LOG_DIR}/homepage_daily_${TIMESTAMP}.log"
...
if [ -f "${ROOT_DIR}/.env" ]; then
    ...
    if [ -r "${ROOT_DIR}/.env" ]; then
        ...
        [ -n "${HOMEPAGE_OUTPUT_BASE_DIR}" ] && echo "    ✓ HOMEPAGE_OUTPUT_BASE_DIR: ${HOMEPAGE_OUTPUT_BASE_DIR}" || echo "    ✗ HOMEPAGE_OUTPUT_BASE_DIR: 미설정"
    else
...
# 필수 스크립트 확인
ORCHESTRATOR_SCRIPT="${ROOT_DIR}/homepage_daily_date_orchestrator.py"
...
# Node.js 스크래퍼 디렉토리 확인
SCRAPER_DIR="${ROOT_DIR}/node/scraper"
if [ -d "${SCRAPER_DIR}" ]; then
    echo "  ✓ 스크래퍼 디렉토리: ${SCRAPER_DIR}"
    # 스크래퍼 스크립트 개수 확인
    SCRAPER_COUNT=$(ls -1 "${SCRAPER_DIR}"/*_scraper.js 2>/dev/null | wc -l)
    echo "  ✓ 발견된 스크래퍼: ${SCRAPER_COUNT}개"
else
    echo "  ✗ ERROR: 스크래퍼 디렉토리를 찾을 수 없음: ${SCRAPER_DIR}"
    exit 1
fi
...
# ============= 9. 날짜 기반 수집 실행 =============
echo "[STEP 9/10] Homepage 날짜 기반 수집 실행..."
echo "==================================================================================="
