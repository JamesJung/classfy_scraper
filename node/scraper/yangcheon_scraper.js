...
class AnnouncementScraper {
            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '#contents', '#contBody', 'table.basic-view', '.basic-view', '.program--contents',
                    ...
                    const fileLinks = document.querySelectorAll('a[onclick*="doUrlDownload"]');

                    fileLinks.forEach(link => {
                        const fileName = link.textContent.trim()
                        const onclick = link.getAttribute('onclick');

                        // The download link is in the href attribute of the <a> tag itself.
                        const url = link.href;
    ...
    async saveAnnouncement(announcement, detailContent) {
        try {
            ...
            if (detailContent.attachments && detailContent.attachments.length > 0) {
                attachmentResults = await this.downloadAttachments(detailContent.attachments, folderPath);
            }

            // content.md 생성 (다운로드 URL 정보 포함)
            const contentMd = this.generateMarkdownContent(announcement, detailContent, attachmentResults);
    ...
    async downloadAttachments(attachments, folderPath) {
        try {
            ...
            for (let i = 0; i < attachments.length; i++) {
                ...
                const downloadInfo = await this.downloadSingleAttachment(attachment, attachDir, i + 1);
                if (downloadInfo) {
                    attachmentResults.push(downloadInfo);
                }
    ...
    async downloadSingleAttachment(attachment, attachDir, index) {
        try {
            ...
            // function doUrlDownload(de_user_file_nm, de_sys_file_nm, de_file_path) {
            ...
            //     location.href = "https://eminwon.yangcheon.go.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=" + enc_de_user_file_nm + "&sys_file_nm=" + enc_de_sys_file_nm + "&file_path=" + enc_de_file_path;
            // }

            // JavaScript 방식 처리
            if (attachment.onclick) {
                // goDownload 패턴 처리
                const goDownloadMatch = attachment.onclick.match(/doUrlDownload\(\s*['"]([^'"]*)['"]\s*,\s*['"]([^'"]*)['"]\s*,\s*['"]([^'"]*)['"]\s*\)/);
                ...
                if (goDownloadMatch) {
                    ...
                    downloadUrl = `https://eminwon.yangcheon.go.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=${enc_user_file_nm}&sys_file_nm=${enc_sys_file_nm}&file_path=${enc_file_path}`;
    ...
    async downloadViaLink(url, attachDir, fileName) {
        try {
            ...
            // SSL 인증서 검증 비활성화를 위한 https 에이전트
            ...
            const response = await axios({
                ...
                responseType: 'stream',
                timeout: 30000,
                httpsAgent: agent,  // HTTPS 에이전트 추가
                headers: {
                    'Accept': '*/*',
     ...
     * 마크다운 컨텐츠 생성
     */
    generateMarkdownContent(announcement, detailContent, attachmentResults = []) {
        const lines = [];
    ...
    async buildDetailUrl(announcement) {
        ...
        if (announcement.link) {
            console.log('onclick 분석 중:', announcement.link);

            //$('#sdmBoardSeq').val(seq);
            // $('#mode').val('view');
            // $('#frm').submit();

            const viewMatch = announcement.link.match(/doSeolContentDeailView\(\s*['"]([^'"]*)['"]\s*\)/);
            if (viewMatch) {
                const detailNo = viewMatch[1]

                let detailUrl = `https://www.yangcheon.go.kr/site/yangcheon/ex/seol/seolContentDeailView.do?not_ancmt_mgt_no=${detailNo}`
                return detailUrl;
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.yangcheon.go.kr/site/yangcheon/ex/seol/seolCollectList.do',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: 'table.basic-list tbody tr'
        })
