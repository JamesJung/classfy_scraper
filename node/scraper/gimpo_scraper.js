            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '#conts', '#contents', 'table.p-table', '.view', '.contents_wrap', '.program--contents',
                    ...
                    const goDownloadLinks = document.querySelectorAll('a.ntfc_file_down');

                    console.log("goDownloadLinks", goDownloadLinks)
                    goDownloadLinks.forEach(link => {
                        // Get the full URL from the href attribute.
                        const userFileName = link.dataset.userFileNm;
                        const sysFileName = link.dataset.sysFileNm;
                        ...
                        const downloadUrl = `https://eminwon.gimpo.go.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=${enc_user_file_nm}&sys_file_nm=${enc_sys_file_nm}&file_path=${enc_file_path}`;

                        if (userFileName && downloadUrl) {
                            attachments.push({
                                name: userFileName,
                                url: downloadUrl,
                            });
        ...
        try {
            ...
            if (attachment.onclick) {
                ...
                if (goDownloadMatch) {
                    ...
                    downloadUrl = `https://eminwon.gimpo.go.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=${enc_user_file_nm}&sys_file_nm=${enc_sys_file_nm}&file_path=${enc_file_path}`;
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.gimpo.go.kr/portal/ntfcPblancList.do?key=1004&cate_cd=1&searchCnd=40900000000',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: '.bbs__list table.p-table tbody tr'
        })
