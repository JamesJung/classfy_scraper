...
현재 구조: prv8/지역1/지역2/공고폴더
목표 구조: prv8/지역1_지역2_공고폴더

실행 방법:
python flatten_prv8.py
"""
...
def flatten_prv(folder_name):
    ...
    # 백업 폴더 생성
    backup_path = Path(f"{folder_name}_backup")
    if not backup_path.exists():
        print("원본 백업을 생성합니다...")
        shutil.copytree(base_path, backup_path)
        print(f"백업 완료: {backup_path}")
    
    print(f"{folder_name.upper()} 폴더 평탄화를 시작합니다...")
...
if __name__ == "__main__":
    ...
    # 커맨드라인 인자 처리
    folder_name = "prv8"  # 기본값
    if len(sys.argv) > 1:
        folder_name = sys.argv[1]
    
    print("=" * 50)
    print(f"{folder_name.upper()} 폴더 평탄화 도구")
    print("=" * 50)
    
    flatten_prv(folder_name)