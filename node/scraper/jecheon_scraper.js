        ...
        while (retries < maxRetries) {
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const fileLinks = document.querySelectorAll('.p-attach a.p-attach__link');
                    const results = [];

                    fileLinks.forEach(downloadLink => {
                        // The file name is the text content of the link, excluding the image alt text.
                        // A simple textContent trim should work.
                        const fileNameElement = downloadLink.querySelector('span:last-of-type');
                        // 파일 이름과 URL을 추출합니다.
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.jecheon.go.kr/www/selectBbsNttList.do?bbsNo=18&key=5233',
            required: true
        })
