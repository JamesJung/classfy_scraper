...
class AnnouncementScraper {
    constructor(options = {}) {
        ...
        this.listSelector = options.listSelector || 'table.bod_list tr';
    ...
    async downloadSingleAttachment(attachment, attachDir, index) {
        try {
            ...
            /*
            실제 다운로드 되는 파일 자바스크립트 함수이다.


            function goDownload(de_user_file_nm, de_sys_file_nm, de_file_path) {
                    var enc_de_user_file_nm = encodeURIComponent(de_user_file_nm);
                    var enc_de_sys_file_nm = encodeURIComponent(de_sys_file_nm);
                    var enc_de_file_path = encodeURIComponent(de_file_path);
                    var url = "https://eminwon.yangsan.go.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=" + enc_de_user_file_nm + "&sys_file_nm=" + enc_de_sys_file_nm + "&file_path=" + enc_de_file_path;
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.yangsan.go.kr/portal/saeol/gosi/list.do?mid=0102010000',
            required: true
        })
