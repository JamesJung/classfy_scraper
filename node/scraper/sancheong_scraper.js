            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '#contents', '#board', '.bbs_default_view', '.contents_wrap', '.program--contents',
                    ...
                    const listItems = document.querySelectorAll('ul.brd_file > li');
                    const attachments = [];

                    listItems.forEach(item => {
                        // Find the anchor tag with the download link
                        const downloadLinkElement = item.querySelector('a[href*="downloadBbsFile"]');

                        // Find the span tag containing the file name
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.sancheong.go.kr/www/selectBbsNttList.do?key=158&bbsNo=118&integrDeptCode=&searchCtgry=%EC%9D%BC%EB%B0%98%EA%B3%B5%EA%B3%A0',
            required: true
        })
        .option('list-selector', {
            ...
            default: 'table.bbs_default_list tbody tr'
        })
