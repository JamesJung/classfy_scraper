...
class GccityScraper extends AnnouncementScraper {
    constructor(options = {}) {
        const defaultOptions = {
            siteCode: 'gccity',
            baseUrl: 'https://www.gccity.go.kr/portal/saeol/gosi/list.do?mId=0301040000',
            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    fileLinks.forEach(item => {
                        const downloadLink = item.querySelector('a[onclick*="goDownload"]');
                        const fileNameSelector = item.querySelector('span');

                        if (downloadLink) {
                            const fileInfo = downloadLink.getAttribute('onclick');
                            const href = downloadLink.href
                            attachments.push({
                                name: fileNameSelector.textContent.trim(),
                                url: href,
                                onclick: fileInfo
                            });
        ...
        try {
            ...
            if (attachment.onclick) {
                ...
                if (goDownloadMatch) {
                    ...
                    downloadUrl = `https://eminwon.gccity.go.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=${enc_user_file_nm}&sys_file_nm=${enc_sys_file_nm}&file_path=${enc_file_path}`;
...
if (require.main === module) {
    ...
    const scraper = new GccityScraper({
