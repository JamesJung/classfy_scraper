...
class AnnouncementScraper {
    ...
    async processAnnouncement(announcement) {
        try {
            ...
            // 1. 리스트에서 날짜 확인
            console.log(`날짜 텍스트 원본: "${announcement.dateText}"`);
            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '#contents', 'tbody.p-table--th-left', '.board-view', '.content_box', '.program--contents',
                        '#user_board_whole',
                    ];
                    ...
                    // 관악구청은 첨부파일이 테이블 내에 있을 수 있음
                    const fileLinks = document.querySelectorAll('a[href*="FileDown"], a[href*="download"], .file a, td a[href*=".pdf"], td a[href*=".hwp"], td a[href*=".xls"], td a[href*=".doc"], td a[href*=".zip"]');
                    ...
                    fileLinks.forEach(link => {
                        ...
                        // "파일보기" 같은 텍스트는 제외
                        if (fileName && !fileName.includes('보기') && (url || onclick)) {
                            attachments.push({
                                name: fileName,
                                url: url,
                                onclick: onclick
    ...
    async downloadAttachments(attachments, folderPath) {
        ...
        try {
            ...
            for (let i = 0; i < attachments.length; i++) {
                ...
                if (result) {
                    // 다운로드 URL 정보 수집
                    downloadUrlInfo[result.fileName || attachment.name] = result.fileFDownUrl || attachment.url;
                }
    ...
    async downloadSingleAttachment(attachment, attachDir, index) {
        try {
            ...
            if (attachment.onclick) {
                // 관악구청 fileDown 패턴 처리
                const fileDownMatch = attachment.onclick.match(/fileDown\(['"]([^'"]+)['"]\s*,\s*['"]([^'"]+)['"]/);
                if (fileDownMatch) {
                    const [, filePath, fileName] = fileDownMatch;
                    // 관악구청 다운로드 URL 패턴
                    downloadUrl = `https://www.gwanak.go.kr/common/download.do?filePath=${encodeURIComponent(filePath)}&fileName=${encodeURIComponent(fileName)}`;
                ...
                // goDownload 패턴 처리
                    ...
                    if (goDownloadMatch) {
                        ...
                        downloadUrl = `https://eminwon.gwanak.go.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=${enc_user_file_nm}&sys_file_nm=${enc_sys_file_nm}&file_path=${enc_file_path}`;
                        actualDownloadUrl = downloadUrl;
            ...
            if (attachment.onclick && attachment.onclick.includes('submit')) {
                ...
                return { fileName: fileName, fileFDownUrl: actualDownloadUrl }
            }
    ...
    async buildDetailUrl(announcement) {
        ...
        // 관악구청 특별 처리: doBbsFView 함수 패턴
        if (announcement.onclick && announcement.onclick.includes('doBbsFView')) {
            // doBbsFView('4474') 형식에서 번호 추출
            const match = announcement.onclick.match(/doBbsFView\(['"]?(\d+)['"]?\)/);
            if (match) {
                const mgtNo = match[1];
                const typeCode = '1';
                const detailUrl = `https://www.gwanak.go.kr/site/gwanak/ex/bbsNew/View.do?not_ancmt_mgt_no=${mgtNo}&typeCode=${typeCode}`;
                console.log('관악구청 상세 URL 생성:', detailUrl);
                return detailUrl;
            }
        }
     ...
     */
        ...
        // YYYYMMDD 형식 체크 (예: 20251024) - 가장 먼저 처리
        const yyyymmddMatch = cleanText.match(/^(\d{4})(\d{2})(\d{2})$/);
        if (yyyymmddMatch) {
            ...
            if (monthNum >= 1 && monthNum <= 12 && dayNum >= 1 && dayNum <= 31) {
                ...
                console.log(`YYYYMMDD 형식 변환: ${dateText} -> ${cleanText}`);
            }
        }
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.gwanak.go.kr/site/gwanak/ex/bbsNew/List.do?typeCode=1',
            required: true
        })
        ...
        .option('title-selector', {
            type: 'string',
            description: '제목 선택자',
            default: 'td.subject a, td.tit a, td:nth-child(3) a'
        })
        .option('date-selector', {
            type: 'string',
            description: '날짜 선택자',
            default: 'td:nth-child(4)'  // 4번째 열이 게시일
        })
        .option('force', {
            alias: 'f',
            type: 'boolean',
