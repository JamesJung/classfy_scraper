                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    if (!dateText) {
                        ...
                        if (announcement && announcement.listDate) {
                            ...
                            const dtElements = document.querySelectorAll('.form-data-info dt');
                            let date = '';

                            // Loop through each <dt> element to find the one with the correct text.
                            for (const dt of dtElements) {
                                if (dt.textContent.trim() === '공고일자') {
                                    // Get the text from the next sibling element (<dd>).
                                    const ddElement = dt.nextElementSibling;
                                    if (ddElement) {
                                        date = ddElement.textContent.trim();
                    ...
                    console.log("attachmentLinks", attachmentLinks, goDownloadLinks)
                    // 두 NodeList를 합치기
                    const allLinks = [...attachmentLinks, ...goDownloadLinks];
                    ...
                    allLinks.forEach(link => {
                        ...
                        //바로보기는 해당 클래스를 가지고 있음
                        if (link.className !== "sbtn_file") {
                            // 중복 제거
            ...
            detailContent.attachments.forEach((att, i) => {
                ...
                if (att.url) {
                    ...
                    // 상대 URL인 경우
                        ...
                        downloadUrl = `${baseUrl}${att.url}`;
                    }
                    // fileSeq 파라미터가 있는 경우 (부산시 특유의 다운로드 URL 구조)
                    else if (att.url.includes('fileSeq=')) {
                        downloadUrl = `${baseUrl}/comm/getFile?${att.url}`;
                    }
                    else {
                        downloadUrl = att.url;
                    }
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.busan.go.kr/nbgosi?curPage=1&conGosiGbn=',
            required: true
        })
