            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '#board', 'table.skinTb-data-resList', '.program--contents',
                    ...
                    const fileContainers = document.querySelectorAll('span.attach_1 a');

                    fileContainers.forEach(container => {

                        const fileName = container.textContent.trim()
                        const downloadLink = container.href
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.gyeongnam.go.kr/index.gyeong?menuCd=DOM_000000135003009000',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: 'table.A-13 > tbody > tr'
        })
