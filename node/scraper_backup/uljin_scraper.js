                ...
                const announcements = await this.page.evaluate((selectors) => {
                    ...
                    rows.forEach((row, index) => {
                        ...
                        if (titleElement && dateElement) {
                            ...
                            const infoText = dateElement.textContent;
                            const dateMatch = infoText.match(/게재일 : (\d{4}-\d{2}-\d{2})/);
            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        'table.p-table', '.over_table', '.content_box', '.program--contents',
                    ...
                    const downloadLinks = document.querySelectorAll('a[title*="다운받기"]');
                    const attachments = [];
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.uljin.go.kr/index.uljin?menuCd=DOM_000000103002007000',
            required: true
        })
        ...
        .option('title-selector', {
            type: 'string',
            description: '제목 선택자',
            default: 'ul.bbs_list > li a strong'
            //default: 'a.a1 strong.t1'
        })
        .option('date-selector', {
            type: 'string',
            description: '날짜 선택자',
            default: 'ul.bbs_list > li a em.info'
