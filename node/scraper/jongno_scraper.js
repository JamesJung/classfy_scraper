...
class AnnouncementScraper {
        ...
        while (retries < maxRetries) {
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const fileLinks = document.querySelectorAll('th.first a');
                    const results = [];
    ...
    async buildDetailUrl(announcement) {
        ...
        if (announcement.link) {
            ...
            //	frm.action="/portal/bbs/selectBoardArticle.do?bbsId=BBSMSTR_000000000271&menuNo=1756&menuId=1756&nttId="+nttId;
            //  frm.submit();
            const viewMatch = announcement.link.match(/viewMove\(\s*['"]([^'"]*)['"]\s*\)/);
            ...
            if (viewMatch) {
                ...
                let viewUrl = `https://www.jongno.go.kr/portal/bbs/selectBoardArticle.do?bbsId=BBSMSTR_000000000271&menuNo=1756&menuId=1756&nttId=${keyValue}`
                return viewUrl
            }
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.jongno.go.kr/portal/bbs/selectBoardList.do?bbsId=BBSMSTR_000000000271&menuId=1756&menuNo=1756',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: 'table.list_type01 tbody tr'
