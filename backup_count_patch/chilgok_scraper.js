        ...
        while (retries < maxRetries) {
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const fileLinks = document.querySelectorAll('#updateFileList a');
                    const results = [];

                    fileLinks.forEach(link => {
                        // 파일 이름은 <a> 태그의 텍스트 콘텐츠에서 가져옵니다.
                        // 이때, 불필요한 공백을 제거합니다.
                        const downloadText = link.querySelector('span');
                        const fileName = downloadText.textContent.trim()

                        const downloadUrl = link ? link.href : null;
        ...
        try {
            ...
            if (attachment.onclick) {
                ...
                if (goDownloadMatch) {
                    ...
                    downloadUrl = `https://eminwon.chilgok.go.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=${enc_user_file_nm}&sys_file_nm=${enc_sys_file_nm}&file_path=${enc_file_path}`;
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.chilgok.go.kr/portal/saeol/gosi/list.do?mId=0201030000',
            required: true
        })
