            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '#content', '.bbs_viewbox', '.contents_wrap', '.program--contents',
                        '.bbs_skin', '.view_box',
                    ...
                    const fileItem = document.querySelectorAll('.attach__item');

                    fileItem.forEach(item => {
                        const fileNameElement = item.querySelector('.text em');
                        const downloadLinkElement = item.querySelector('.attach_btn.down');
                        ...
                        let downloadLink = '다운로드 링크 정보 없음';
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.chungbuk.go.kr/www/selectGosiPblancList.do?key=422',
            required: true
        })
