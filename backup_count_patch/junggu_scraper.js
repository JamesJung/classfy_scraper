            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        'table.p-table', '.board_view_02', '.content_box', '.program--contents',
                    ...
                    const fileParagraphs = document.querySelectorAll('p.mb05');

                    fileParagraphs.forEach(p => {
                        // Find the anchor tag with the "파일 다운로드" title within each paragraph.
                        const downloadLink = p.querySelector('a[title="파일 다운로드"]');

                        if (downloadLink) {
                            // Get the file name from the text content of the link.
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.junggu.seoul.kr/content.do?cmsid=14232',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: '.board_list table tbody tr'
