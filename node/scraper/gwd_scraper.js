            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '.s-v-board-default', 'table.skinTb-data-resList', '.program--contents',
                    ...
                    const fileContainers = document.querySelectorAll('.attachFile');

                    console.log("fileContainers", fileContainers)
                    fileContainers.forEach(container => {
                        const downloadLinkElement = container.querySelector('a[href*="/download"]');
                        const textNodes = Array.from(downloadLinkElement.childNodes).filter(node => node.nodeType === Node.TEXT_NODE);
                        const downloadLink = downloadLinkElement.href;

                        // The file name is typically the first or only non-empty text node
                        const fileNameTextNode = textNodes.find(node => node.textContent.trim().length > 0);

                        if (fileNameTextNode) {
                            fileName = fileNameTextNode.textContent.trim();
                        }
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://state.gwd.go.kr/portal/bulletin/notification',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: 'table.skinTb-data-resList > tbody > tr'
        })
