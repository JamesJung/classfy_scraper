...
class AnnouncementScraper {
                ...
                const announcements = await this.page.evaluate((selectors) => {
                    ...
                    rows.forEach((row, index) => {
                        ...
                        if (titleElement && dateElement) {
                            let title = titleElement.textContent.trim().replace("새 글", "");
                            title = title.replace(/\s/g, "");

                            // 날짜 추출 - 공고기간이 있는 경우와 없는 경우 처리
                            let dateText = '';
                            console.log("dateElement", dateElement)
                            const wrapElement = row.querySelector('i.wrap1t3');
                            if (wrapElement) {
                                const t3Elements = wrapElement.querySelectorAll('span.t3');

                                console.log("t3Elements", t3Elements)
                                // 공고기간이 있는지 확인
                                let hasAnnouncementPeriod = false;
                                let actualDateIndex = 0;

                                for (let i = 0; i < t3Elements.length; i++) {
                                    const text = t3Elements[i].textContent.trim();
                                    if (text.includes('공고기간')) {
                                        hasAnnouncementPeriod = true;
                                        actualDateIndex = i + 1; // 다음 요소가 실제 날짜
                                        break;
                                    }
                                }

                                // 실제 날짜 추출
                                if (hasAnnouncementPeriod && actualDateIndex < t3Elements.length) {
                                    dateText = t3Elements[actualDateIndex].textContent.trim();
                                } else if (t3Elements.length > 0) {
                                    // 공고기간이 없으면 첫 번째 t3가 날짜
                                    dateText = t3Elements[0].textContent.trim();
                                }
                                dateText = dateText.replace("등록일:", '').trim();
                                console.log("DATe", dateText)
                            } else {
                                console.log("wrap1t3가 없는 경우 기존 방식")
                                // wrap1t3가 없는 경우 기존 방식
                                dateText = dateElement.textContent.replace("등록일:", '').trim();
                            }
                            // 다양한 방식으로 링크 정보 추출
            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '#contents', '#board', '.bbs1view1', '.contents_wrap', '.program--contents',
                    ...
                    const downloadAnchor = document.querySelectorAll('a.filename');
                    downloadAnchor.forEach(link => {
    ...
    buildListUrl(pageNum) {
        ...
        url.searchParams.set('cpage', pageNum);
        return url.toString();
    }
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.sacheon.go.kr/news/00009/00014.web',
            required: true
        })
        ...
        .option('date-selector', {
            ...
            default: 'a.a1 i.wrap1t3 span.t3'
        })
        .option('count', {
            alias: 'c',
            type: 'boolean',
