        ...
        while (retries < maxRetries) {
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const fileItems = document.querySelectorAll('ul.board_file li');

                    fileItems.forEach(item => {
                        // 파일 이름을 포함하는 <em> 태그를 찾습니다.
                        const fileNameElement = item.querySelector('em');
                        // 다운로드 링크를 포함하는 <a> 태그를 찾습니다.
                        const downloadLink = item.querySelector('a.down');
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.jindo.go.kr/home/gosi/general.cs?m=878',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: 'table.gosi_list tbody tr'
        })
