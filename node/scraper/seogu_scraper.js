...
class AnnouncementScraper {
        ...
        while (retries < maxRetries) {
            try {
                ...
                if (pageNum === 1) {
                    ...
                    // 페이지 2 이상인 경우 JavaScript 함수를 통해 이동
                    console.log(`페이지 ${pageNum}로 이동 중 (JavaScript goPage 함수 사용)...`);

                    // 먼저 페이지 1로 이동 (세션 유지를 위해)
                    if (!this.page.url().includes('mid=a10807010000')) {
                        const baseUrl = this.buildListUrl(1);
                    ...
                    // JavaScript의 goPage 함수 실행과 네비게이션 대기를 동시에 처리
                    await Promise.all([
                        this.page.waitForNavigation({
                            waitUntil: 'networkidle',
                            timeout: 30000
                        }),
                        this.page.evaluate((pageNum) => {
                            if (typeof goPage === 'function') {
                                goPage(pageNum);
                            } else {
                                // goPage 함수가 없는 경우 폼 직접 조작
                                const form = document.querySelector('form[name="srhForm"]');
                                if (form) {
                                    const nPageInput = form.querySelector('input[name="nPage"]');
                                    if (nPageInput) {
                                        nPageInput.value = pageNum;
                                        form.submit();
                                    }
                                }
                            }
            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        'table.p-table', '.board_view', '.content_box', '.program--contents',
                    ...
                    fileItems.forEach(item => {
                        // Get the file name from the text content of the list item.
                        const fileNameText = item.textContent.trim();
                        // The first part of the text content is the file name. We need to clean it up.
                        // It's the text content before the '다운로드' button.
                        const fileName = fileNameText.substring(0, fileNameText.indexOf('다운로드')).trim();

                        // Find the download anchor tag.
                        const downloadAnchor = item.querySelector('a.btn_line');
                        if (downloadAnchor) {
                            // Get the onclick attribute's value which contains the link parameters.
                            const onclickAttr = downloadAnchor.getAttribute('href');
                            // The link is a javascript call, so we will use the parameters to reconstruct the file path.
                            // In a real-world scenario, you might need more complex logic to handle the `goDownLoad` function.
                            // For this example, we'll just return the javascript call as the link.
                            const downloadLink = onclickAttr;
    ...
    buildListUrl(pageNum) {
        // Seogu uses nPage parameter for pagination
        const url = new URL(this.baseUrl);
        url.searchParams.set('nPage', pageNum);
        return url.toString();
    }
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.seogu.gwangju.kr/menu.es?mid=a10807010000',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: 'table.tstyle_list tbody tr'
