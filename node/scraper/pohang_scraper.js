        ...
        while (retries < maxRetries) {
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const fileLinks = document.querySelectorAll('a[onclick*="goDownload"]');
                    const results = [];

                    fileLinks.forEach(link => {
                        // 파일 이름은 <a> 태그의 텍스트 콘텐츠에서 가져옵니다.
                        // 이때, 불필요한 공백을 제거합니다.
                        const fileName = link.querySelector("span").textContent.trim()
                        const url = link.href
        ...
        try {
            ...
            if (attachment.onclick) {
                ...
                if (goDownloadMatch) {
                    ...
                    downloadUrl = `https://eminwon.pohang.go.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=${enc_user_file_nm}&sys_file_nm=${enc_sys_file_nm}&file_path=${enc_file_path}`;
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.pohang.go.kr/portal/saeol/gosi/list.do?mid=0202010000',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: 'table.board_list tbody tr, table.bod_list tbody tr'
