            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        'table.bbs_default', '.bbs__view', '.content_box', '.program--contents',
                    ...
                    const fileItem = document.querySelector('ul.view_attach li');

                    if (fileItem) {
                        // Find the span that holds the file name and remove the image alt text
                        const fileNameSpan = fileItem.querySelector('.down_view span');
                        const fileName = fileNameSpan.textContent.trim();

                        // Find the download link and get its href attribute
                        const downloadLink = fileItem.querySelector('.file_down');
    ...
    async downloadSingleAttachment(attachment, attachDir, index) {
        try {
            ...
            if (attachment.onclick && attachment.onclick.includes('submit')) {
                ...
                // 일반 링크 방식
                console.log("일반 링크 방식-----", downloadUrl)
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.taebaek.go.kr/www/selectBbsNttList.do?bbsNo=25&key=352',
            required: true
        })
