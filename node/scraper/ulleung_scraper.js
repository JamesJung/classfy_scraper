            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        'table.p-table', '.boardView', '.content_box', '.program--contents',
                    ...
                    const downloadLinks = document.querySelectorAll('a[title*="파일 다운로드"]');
                    const attachments = [];

                    downloadLinks.forEach(link => {
                        // Get the file name from the text content of the link and trim any whitespace.
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.ulleung.go.kr/ko/page.do?mnu_uid=571&boardType=notice',
            required: true
        })
        .option('list-selector', {
            type: 'string',
            description: '리스트 선택자',
            default: '.boardWrap table.tbl_board tbody tr'
