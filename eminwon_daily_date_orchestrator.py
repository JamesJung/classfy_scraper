...
"""
Eminwon 날짜 기반 일일 수집 오케스트레이터

eminwon_site_announcement_date 테이블을 기반으로 
각 site의 latest_announcement_date 이후 데이터를 수집하는 시스템

1. eminwon_site_announcement_date 테이블에서 site_code, host_url, latest_announcement_date 읽기
2. 각 스크래퍼를 날짜 옵션으로 실행
...
class EminwonDailyDateOrchestrator:
    def __init__(self, test_mode=False, specific_sites=None, verbose=False, max_workers=3, override_date=None):
        ...
        output_base = os.environ.get("EMINWON_OUTPUT_BASE_DIR", "batch_eminwon_result")
    ...
    def setup_logging(self):
        ...
        log_file = log_dir / f'eminwon_daily_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
    ...
    def get_sites_to_process(self):
        """Get sites from eminwon_site_announcement_date table"""
    ...
    def extract_collected_count(self, output_text):
        ...
        try:
            # Look for patterns from Node.js eminwon_scraper.js output
            patterns = [
                r'처리된 공고 수:\s*(\d+)',  # Node.js scraper main pattern
                r'(\d+)개 수집 완료',
                r'(\d+)개 공고 수집',
                r'수집된 공고:\s*(\d+)개?',
                r'총\s*(\d+)개의 공고',
    ...
    def run(self):
        ...
        self.logger.info("=" * 80)
        self.logger.info("Starting Eminwon Daily Date-based Collection")
...
def main():
    parser = argparse.ArgumentParser(description="Eminwon Daily Date-based Orchestrator")
    parser.add_argument("--test", action="store_true", help="Run in test mode")
    parser.add_argument(
        "--sites",
        nargs="+",
        help="Specific sites to process (e.g., --sites 청주시 가평군)",
    )
    parser.add_argument(
        "--workers",
        type=int,
    ...
    orchestrator = EminwonDailyDateOrchestrator(
        test_mode=args.test,
