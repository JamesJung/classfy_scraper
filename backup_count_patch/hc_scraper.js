        ...
        while (retries < maxRetries) {
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const listItems = document.querySelectorAll('.attach1 li');
                    const attachments = [];

                    listItems.forEach(item => {
                        // 각 <li> 요소 내에서 'filename' 클래스를 가진 <a> 태그를 찾습니다.
                        const fileAnchor = item.querySelector('a.filename');

                        if (fileAnchor) {
                            // 파일 이름은 <a> 태그의 텍스트 콘텐츠입니다.
                            const fileName = fileAnchor.textContent.trim();
                            // 다운로드 링크는 해당 <a> 태그의 href 속성입니다.
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.hc.go.kr/04923/04924/04948.web',
            required: true
        })
