...
class AnnouncementScraper {
                    ...
                    for (const announcement of announcements) {
                        try {
                            ...
                            const saved = await UrlManager.saveDetailUrl({
                                ...
                                list_date: announcement.dateText || announcement.date || '',
    ...
    async processAnnouncement(announcement) {
        try {
            ...
            if (listDate && listDate.date() < this.targetDate) {
            ...
            // 4. 상세 페이지에서 날짜 재확인
            if (detailContent.date && detailContent.date() < this.targetDate) {
            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    const contentSelectors = [
                        '#contents', '.content', '.program--contents', '.main', '.article', '.post',
                        '#main', '#article', '#post',
                    ...
                    const attachmentLinks = document.querySelectorAll('a.p-attach__link');

                    attachmentLinks.forEach(link => {
                        // Get the onclick attribute's value
                        const hrefAttribute = link.getAttribute('href');

                        // Use a regular expression to parse the goDownLoad() function arguments
                        const regex = /goDownLoad\('([^']+)','([^']+)','([^']+)'\);/;
                        const matches = hrefAttribute.match(regex);

                        if (matches && matches.length === 4) {
                            const fileName = matches[1]; // First argument: user_file_nm
                            const systemFileName = matches[2]; // Second argument: sys_file_nm
                            const filePath = matches[3]; // Third argument: file_path

                            // Construct the final download URL
                            const url = `https://eminwon.cheongju.go.kr/emwp/jsp/ofr/FileDown.jsp?user_file_nm=${encodeURIComponent(fileName)}&sys_file_nm=${encodeURIComponent(systemFileName)}&file_path=${encodeURIComponent(filePath)}`;
    ...
    async saveAnnouncement(announcement, detailContent) {
        try {
            ...
            if (detailContent.attachments && detailContent.attachments.length > 0) {
                attachmentUrl = await this.downloadAttachments(detailContent.attachments, folderPath);
            }
    ...
    async downloadAttachments(attachments, folderPath) {
        try {
            ...
            for (let i = 0; i < attachments.length; i++) {
                ...
                const downUrl = await this.downloadSingleAttachment(attachment, attachDir, i + 1);
                if (downUrl.fileName) {
                    attachmentUrl[downUrl.fileName] = downUrl.fileUrl
                }
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'https://www.cheongju.go.kr/www/selectEminwonNoticeList.do?key=281&nowDongGn=&notAncmtSeCd=04',
            required: true
        })
        .option('go-page', {
            alias: 'p',
...
async function main() {
    ...
    if (argv.count) {
        ...
        const UrlManager = require('./url_manager');
        const stats = await UrlManager.getStats(argv.site, argv.batchDate || require('moment')().format('YYYY-MM-DD'));
