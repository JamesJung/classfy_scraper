...
class AnnouncementScraper {
            ...
            try {
                ...
                const content = await this.page.evaluate((options) => {
                    ...
                    if (!dateText) {
                        ...
                        const postInfo = document.querySelector('#listGosiReg');
                        dateText = postInfo.textContent.trim()
        ...
        try {
            ...
            await this.page.evaluate((params) => {
                ...
                if (typeof fn_egov_downFile === 'function') {
                    ...
                    form.method = 'post';
                    form.action = 'http://eminwon.suncheon.go.kr/emwp/jsp/ofr/FileDownNew.jsp';
            ...
            const downloadPromise = new Promise((resolve, reject) => {
                ...
                const downloadHandler = async (download) => {
                    try {
                        ...
                        // // 다운로드가 완료될 때까지 대기 (최대 10초, 100ms 간격으로 확인)
                        // let tempPath;
                        // let attempts = 0;
                        ...
                        //             console.log(`✅ 임시 파일 확인됨: ${tempPath} (${attempts + 1}번째 시도)`);
                        //             break;
                        //         }
                        //     } catch (pathError) {
                        //         console.log(`임시 파일 경로 확인 중... (${attempts + 1}/${maxAttempts})`);
                        //     }
                        ...
                        // if (!tempPath || !(await fs.pathExists(tempPath))) {
                        //     throw new Error(`임시 파일 접근 실패 (${maxAttempts}회 시도): ${tempPath}`);
                        // }
    ...
    //         // Playwright를 사용한 브라우저 다운로드 (세션 유지)
    //         await this.downloadViaBrowser(downloadUrl, attachDir, fileName);
     ...
     * 브라우저를 통한 다운로드 (세션 유지)
     */
    // async downloadViaBrowser(url, attachDir, fileName) {
    //     try {
    //         console.log(`브라우저 다운로드 시작: ${url}`);
    ...
    //         } catch (timeoutError) {
    //             // 다운로드 이벤트가 발생하지 않은 경우, 페이지 응답을 직접 처리
    //             console.log('다운로드 이벤트 감지 실패, 응답 직접 처리 시도...');

    //             const response = await this.page.evaluate(async (downloadUrl) => {
    //                 console.log('브라우저에서 다운로드 URL 요청:', downloadUrl);
    ...
    //                     console.log('응답 내용 미리보기 (처음 200자):', text.substring(0, 200));

    //                     // JavaScript 오류 메시지 확인
    //                     if (text.includes('alert(') || text.includes('history.back')) {
    //                         console.log('JavaScript 오류 응답 감지');
    //                         return {
    //                             success: false,
    //                             error: 'JavaScript error response',
    //                             content: text
    //                         };
    //                     }
    ...
    //             console.log('fetch 응답 결과:', response);
    ...
    //                 // 바이너리 데이터를 버퍼로 변환하여 저장
    //                 const buffer = Buffer.from(response.data);
    //                 await fs.writeFile(filePath, buffer);

    //                 console.log(`직접 다운로드 완료: ${cleanFileName} (크기: ${response.size} bytes)`);
    //                 return;
    //             } else if (response.error === 'JavaScript error response') {
    //                 console.error('JavaScript 오류 응답 받음:', response.content);
    //                 throw new Error(`다운로드 실패: ${response.error} - 파일을 찾을 수 없거나 접근 권한 없음`);
    //             } else {
    //                 throw new Error(`다운로드 실패: ${response.status} ${response.statusText}`);
    //             }
    //         }

    //         // Playwright 다운로드 이벤트 처리
    //         const suggestedFilename = download.suggestedFilename();
    ...
    //         console.log(`Playwright 다운로드 완료: ${cleanFileName}`);

    //     } catch (error) {
    //         throw new Error(`브라우저 다운로드 실패: ${error.message}`);
    //     }
    // }
    ...
    //         console.log("downloadViaLink", url)
...
function setupCLI() {
    return yargs
        ...
        .option('url', {
            ...
            type: 'string',
            description: '기본 URL',
            default: 'http://www.suncheon.go.kr/kr/news/0004/0005/0001/',
            required: true
        })
